...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 14520048)) \
        and (___check_obj_id(L['___stack0'], 13593536)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 130213600)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140426727373504))) \
        and (___compile_config_hash() == 'd93dc618b8cd5753c67903c246f1ff65') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140429980938208)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 14520048)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 13593536)) \
        and (___check_type_id(L['cloned_inputs'][0], 75264704)) \
        ...
        and (___compile_config_hash() == 'd93dc618b8cd5753c67903c246f1ff65') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_models_dot_crossvit'].List, 140432113849712)) \
        and (___check_obj_id(G['__import_timm_dot_models_dot_crossvit'].scale_image.__defaults__[0], 7677632)) \
