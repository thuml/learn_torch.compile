...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 17993008)) \
        and (___check_obj_id(L['___stack0'], 17066496)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 133709696)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140136145107648))) \
        and (___compile_config_hash() == '035c3b2ca3ad5d2047352e5f69a8747e') \
        and (not ___needs_nopython()) \
