...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 13262672)) \
        and (___check_obj_id(L['___stack0'], 12336160)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 128974736)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140645761694400))) \
        and (___compile_config_hash() == 'cd189614ea461f8ecd0dcf6af59038dc') \
