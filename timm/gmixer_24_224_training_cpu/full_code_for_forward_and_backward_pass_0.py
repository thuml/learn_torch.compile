...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 15023856)) \
        and (___check_obj_id(L['___stack0'], 14097344)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 130710064)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139798099229376))) \
        and (___compile_config_hash() == 'fd72446455433b45fe8fa2e24bc74156') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139801353052864)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 15023856)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 14097344)) \
        and (___check_type_id(L['cloned_inputs'][0], 75769584)) \
