...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140051289954224)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 35851920)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 1) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 34925408)) \
        and (___check_obj_id(L['mod'].forward_head.__defaults__[0], 7677632)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140050313820864))) \
        and (___compile_config_hash() == 'dc9ce9e7115f682e09545c3faa82a584') \
...
def __transformed_code_0_for_forward_pass(self, mod, inputs, collect_outputs):
    ...
    getattr(mod.stages, '2'
        ).downsample.attn_downsample.attention_bias_cache.update({'cpu':
        graph_out_0[10]})
