...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 31407952)) \
        and (___check_obj_id(L['___stack0'], 30481440)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 147088192)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140436420999872))) \
        and (___compile_config_hash() == 'f3563e292a5fe9934ad2dc1625a93d94') \
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_46:
        pred = mod(*cloned_inputs)
        if isinstance(pred, tuple):
            pred = pred[0]
        loss = self.compute_loss(pred)
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_58:
        pred = mod(*cloned_inputs)
        if isinstance(pred, tuple):
            pred = pred[0]
        loss = self.compute_loss(pred)
