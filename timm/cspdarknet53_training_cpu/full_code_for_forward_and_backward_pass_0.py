...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 22351600)) \
        and (___check_obj_id(L['___stack0'], 21425088)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 138077920)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139815107623616))) \
        and (___compile_config_hash() == '81e4dd6d18e8a280ee24da0d5137b57c') \
