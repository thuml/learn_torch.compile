...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 18915152)) \
        and (___check_obj_id(L['___stack0'], 17988640)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 134594592)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140185882442432))) \
        and (___compile_config_hash() == '2d65094fa6c5a0aa50c29e13b21139a4') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140189152821504)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 18915152)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 17988640)) \
        and (___check_type_id(L['cloned_inputs'][0], 79666912)) \
