...
def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 42609968)) \
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 42609968)) \
        and (___check_obj_id(L['___stack0'], 41683456)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 158281456)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140226435627712))) \
        and (___compile_config_hash() == 'e9ebdfd1b277a9746b56c6c4111c726f') \
        and (not ___needs_nopython()) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140229691175168)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 42609968)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 41683456)) \
        and (___check_type_id(L['cloned_inputs'][0], 103356768)) \
        ...
        and (___compile_config_hash() == 'e9ebdfd1b277a9746b56c6c4111c726f') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_models_dot_crossvit'].List, 140231823938928)) \
