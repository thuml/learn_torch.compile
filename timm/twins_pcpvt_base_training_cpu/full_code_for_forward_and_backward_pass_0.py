...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 26373968)) \
        and (___check_obj_id(L['___stack0'], 25447456)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 142062304)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139865315277504))) \
        and (___compile_config_hash() == '0d7fdf52218b3d6e43d2c28b54c06d40') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139868569080864)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 26373968)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 25447456)) \
        and (___check_type_id(L['cloned_inputs'][0], 87117520)) \
