...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 25497328)) \
        and (___check_obj_id(L['___stack0'], 24570816)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 141187024)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140022918085312))) \
        and (___compile_config_hash() == 'f7f3c0d0b7c9b7bea83819f707af71b8') \
