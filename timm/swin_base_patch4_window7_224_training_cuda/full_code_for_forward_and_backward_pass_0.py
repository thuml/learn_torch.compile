...
def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 18877968)) \
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 18877968)) \
        and (___check_obj_id(L['___stack0'], 17951456)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 134597824)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140165330942656))) \
        and (___compile_config_hash() == 'fd8f5a95ffd4dd1ab7e26959aad66354') \
        and (not ___needs_nopython()) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140168601350256)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 18877968)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 17951456)) \
        and (___check_type_id(L['cloned_inputs'][0], 79622304)) \
        ...
        and (___compile_config_hash() == 'fd8f5a95ffd4dd1ab7e26959aad66354') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_format'].Format.NHWC, 140165420630864)) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_patch_embed'].Format.NCHW, 140165420630752)) \
