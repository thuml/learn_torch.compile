...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 16363152)) \
        and (___check_obj_id(L['___stack0'], 15436640)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 132042912)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139817276275392))) \
        and (___compile_config_hash() == 'e8141c6c39a48ea5decc4e61a75fdc58') \
