...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 23092976)) \
        and (___check_obj_id(L['___stack0'], 22166464)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 138791392)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139914466360000))) \
        and (___compile_config_hash() == '91f7fc2ae7309c14dafcc75d465d943b') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139914465454368)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 23092976)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 22166464)) \
        and (___check_type_id(L['cloned_inputs'][0], 83836928)) \
