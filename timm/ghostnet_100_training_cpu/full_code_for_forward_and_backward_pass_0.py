...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 30297840)) \
        and (___check_obj_id(L['___stack0'], 29371328)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 146014032)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140575122438848))) \
        and (___compile_config_hash() == '6176aaaf4effd5b2f9e0db5f0f00f02c') \
