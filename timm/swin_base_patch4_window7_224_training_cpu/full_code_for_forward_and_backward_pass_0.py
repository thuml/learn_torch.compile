...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 22892624)) \
        and (___check_obj_id(L['___stack0'], 21966112)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 138614208)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139879259186816))) \
        and (___compile_config_hash() == '1d9eaa955012bb88497e4e79e73d3c29') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139880236832688)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 22892624)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 21966112)) \
        and (___check_type_id(L['cloned_inputs'][0], 83636160)) \
        ...
        and (___compile_config_hash() == '1d9eaa955012bb88497e4e79e73d3c29') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_format'].Format.NHWC, 139879349092976)) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_patch_embed'].Format.NCHW, 139879349092864)) \
