...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 19177200)) \
        and (___check_obj_id(L['___stack0'], 18250688)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 134868064)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140698528709312))) \
        and (___compile_config_hash() == '92e4a60280830e5abb6855783e17f78d') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140701783984592)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 19177200)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 18250688)) \
        and (___check_type_id(L['cloned_inputs'][0], 79921856)) \
