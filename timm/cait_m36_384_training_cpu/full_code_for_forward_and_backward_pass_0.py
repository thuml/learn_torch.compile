...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 20750064)) \
        and (___check_obj_id(L['___stack0'], 19823552)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 136429760)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139670974315200))) \
        and (___compile_config_hash() == 'bbf87bd101eac27e4a4e641d58c557ea') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139673901120048)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 20750064)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 19823552)) \
        and (___check_type_id(L['cloned_inputs'][0], 81496736)) \
