...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 13581968)) \
        and (___check_obj_id(L['___stack0'], 12655456)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 129286032)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140466435062464))) \
        and (___compile_config_hash() == 'a4d4ac11644ca5b703fdef7095a88e6c') \
