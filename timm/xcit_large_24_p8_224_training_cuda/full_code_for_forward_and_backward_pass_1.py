...
def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 19529328)) \
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 19529328)) \
        and (___check_obj_id(L['___stack0'], 18602816)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 135193648)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139615923447424))) \
        and (___compile_config_hash() == '64600e9bd336196cc7ee807255f13eba') \
        and (not ___needs_nopython()) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139615922529920)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 19529328)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 18602816)) \
        and (___check_type_id(L['cloned_inputs'][0], 80274432)) \
        ...
        and (___compile_config_hash() == '64600e9bd336196cc7ee807255f13eba') \
        and (not ___needs_nopython()) \
        and (___check_type_id(G['__import_timm_dot_models_dot_xcit'].torch.float32, 139621176227584)) \
