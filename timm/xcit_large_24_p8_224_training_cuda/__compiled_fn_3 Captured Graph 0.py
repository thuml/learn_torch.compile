...
def forward(self, L_cloned_inputs_0_ : torch.Tensor):
    ...
    l__mod___patch_embed_proj_0_0 = self.L__mod___patch_embed_proj_0_0(l_cloned_inputs_0_);  l_cloned_inputs_0_ = None
    ...
    arange = torch.arange(1, 29, dtype = torch.float32, device = device(type='cuda', index=0))
    ...
    arange_1 = torch.arange(1, 29, dtype = torch.float32, device = device(type='cuda', index=0))
    ...
    dim_t = torch.arange(32, dtype = torch.float32, device = device(type='cuda', index=0))
    ...
    pred = self.L__mod___head(x_487);  x_487 = None
    return (pred,)
