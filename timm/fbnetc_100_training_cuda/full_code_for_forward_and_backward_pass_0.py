...
def __resume_at_120_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
...
def transformed___resume_at_120_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_120_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_120_9 ============#

def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 32779472)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139971341777600))) \
        and (___compile_config_hash() == '8ac6951f4aa3560abc9487c0b4f3887b') \
...
def __transformed_code_3_for_resume_in_forward_and_backward_pass(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_120_9(self.optimizer_step(), mod, collect_outputs,
        cloned_inputs, pred, loss)


# Note: if there is a transformed version below, this function might well not be executed directly. Please check the transformed version if possible.
def __resume_at_156_8(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    loss = self.compute_loss(pred)
    ___stack0(None, None, None)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def transformed___resume_at_156_8(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_156_8(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_156_8 ============#

def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 32779472)) \
        and (___check_obj_id(L['___stack0'], 31852960)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 148484048)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139971341777600))) \
        and (___compile_config_hash() == '8ac6951f4aa3560abc9487c0b4f3887b') \
        and (not ___needs_nopython()) \
