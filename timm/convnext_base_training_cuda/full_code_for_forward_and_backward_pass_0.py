...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 31452464)) \
        and (___check_obj_id(L['___stack0'], 30525952)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 147139216)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140581147605696))) \
        and (___compile_config_hash() == '5471b3b790f028ef02fa5e0a8305f3c0') \
        and (not ___needs_nopython()) \
