...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 13634768)) \
        and (___check_obj_id(L['___stack0'], 12708256)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 129320400)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139718527220416))) \
        and (___compile_config_hash() == '430ed60617f3a6ac1b929ec46e83a7e8') \
        and (not ___needs_nopython()) \
