...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 21139280)) \
        and (___check_obj_id(L['___stack0'], 20212768)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 136810832)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139627015715520))) \
        and (___compile_config_hash() == '78e59df36f240ca73f023e35ebcc095a') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___compile_config_hash() == '78e59df36f240ca73f023e35ebcc095a') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_patch_embed'].Format.NCHW, 139627105420000)) \
