...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 30502832)) \
        and (___check_obj_id(L['___stack0'], 29576320)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 146167712)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139867297725120))) \
        and (___compile_config_hash() == '3dc1dc03c4c8294733c9c3ebd3d196cc') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139870476877440)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 30502832)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 29576320)) \
        and (___check_type_id(L['cloned_inputs'][0], 91247312)) \
