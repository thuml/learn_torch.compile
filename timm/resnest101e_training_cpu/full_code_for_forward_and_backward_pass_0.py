...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 28741360)) \
        and (___check_obj_id(L['___stack0'], 27814848)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 144448176)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139684388698816))) \
        and (___compile_config_hash() == '7f1adeb3b4f3f6ca095445e77e9e6869') \
