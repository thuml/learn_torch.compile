...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 37675056)) \
        and (___check_obj_id(L['___stack0'], 36748544)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 153389104)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139807340963456))) \
        and (___compile_config_hash() == '3cbf75423eaed1f67478a89b19622e13') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139810435290816)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 37675056)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 36748544)) \
        and (___check_type_id(L['cloned_inputs'][0], 98418976)) \
        ...
        and (___compile_config_hash() == '3cbf75423eaed1f67478a89b19622e13') \
        and (not ___needs_nopython()) \
        and (___check_type_id(G['__import_timm_dot_models_dot_xcit'].torch.float32, 139812593620736)) \
