...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 34799344)) \
        and (___check_obj_id(L['___stack0'], 33872832)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 150486240)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140618662755008))) \
        and (___compile_config_hash() == '47d5bbfa226e94eeea79245a0ebf33dc') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140621506796736)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 34799344)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 33872832)) \
        and (___check_type_id(L['cloned_inputs'][0], 95545616)) \
        ...
        and (___compile_config_hash() == '47d5bbfa226e94eeea79245a0ebf33dc') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_patch_embed'].Format.NCHW, 140618753032928)) \
