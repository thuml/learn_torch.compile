...
def __resume_at_114_10(___stack0):
    foreach = False
    if foreach:
...
def transformed___resume_at_114_10(___stack0):
    L = {"___stack0": ___stack0}
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_114_10(___stack0)

#============ end of __resume_at_114_10 ============#
...
def __guard_0_for_sgd(L):
    return (___guarded_code.valid) \
        ...
        and (len(L['momentum_buffer_list']) == 148) \
...
def __transformed_code_0_for_sgd(params, d_p_list, momentum_buffer_list, has_sparse_grad, foreach, weight_decay, momentum, lr, dampening, nesterov, maximize):
    ...
    return __resume_at_114_10(_multi_tensor_sgd(params, d_p_list,
...
def sgd(params, d_p_list, momentum_buffer_list, has_sparse_grad, foreach, weight_decay, momentum, lr, dampening, nesterov, maximize):
    if foreach is None:
        if not torch.jit.is_scripting():
            __temp_89 = _default_to_fused_or_foreach(params, differentiable=
