...
def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 15346992)) \
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 15346992)) \
        and (___check_obj_id(L['___stack0'], 14420480)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 131030656)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140089636780736))) \
        and (___compile_config_hash() == '90e7392fc32ebccb3d6ff8eba99c5728') \
        and (not ___needs_nopython()) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140090612815792)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 15346992)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 14420480)) \
        and (___check_type_id(L['cloned_inputs'][0], 76091808)) \
        ...
        and (___compile_config_hash() == '90e7392fc32ebccb3d6ff8eba99c5728') \
        and (not ___needs_nopython()) \
        and (___check_obj_id(G['__import_timm_dot_layers_dot_patch_embed'].Format.NCHW, 140089724781280)) \
