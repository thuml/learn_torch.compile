...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 36818672)) \
        and (___check_obj_id(L['___stack0'], 35892160)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 152531360)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140065445919424))) \
        and (___compile_config_hash() == 'a92b4726eb4495571326abf56fbf8429') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140068701946544)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 36818672)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 35892160)) \
        and (___check_type_id(L['cloned_inputs'][0], 97563328)) \
