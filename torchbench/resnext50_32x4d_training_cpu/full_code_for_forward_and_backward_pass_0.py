...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 94022176)) \
        and (___check_obj_id(L['___stack0'], 11263296)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 127972896)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140397369057504))) \
        and (___compile_config_hash() == 'f034fcf822e9ad7d4c7ce90903d6e462') \
