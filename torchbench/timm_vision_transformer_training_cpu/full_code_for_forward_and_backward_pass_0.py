...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 110712608)) \
        and (___check_obj_id(L['___stack0'], 27954688)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 144622832)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140392633819360))) \
        and (___compile_config_hash() == 'b9a06c7a0398c6b554697145e1636bf9') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140395559483520)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 110712608)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 27954688)) \
        and (___check_type_id(L['cloned_inputs'][0], 89495872)) \
