...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140157871754448)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 93919392)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 1) \
        and (___check_type_id(L['inputs'][0], 72701760)) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 11160256)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140155033411808))) \
        and (___compile_config_hash() == 'b5d7c027fe0e75a09e7bfcc93f320cb7') \
