...
def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 93131232)) \
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 93131232)) \
        and (___check_obj_id(L['___stack0'], 10371792)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 127043552)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140476903039200))) \
        and (___compile_config_hash() == 'c84f233ec8813971a4668ec643606d7f') \
        and (not ___needs_nopython()) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140479782238336)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 93131232)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 2) \
        and (___check_obj_id(L['self'].autocast, 10371792)) \
        and (___check_type_id(L['cloned_inputs'][0], 71914416)) \
        and (hasattr(L['cloned_inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs'][1], 71914416)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_bart_dot_modeling_bart'].torch.long, 140482065946368)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_bart_dot_modeling_bart'].torch.float16, 140482065946368)) \
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_42:
        pred = mod(*cloned_inputs)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_53:
        pred = mod(*cloned_inputs)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
