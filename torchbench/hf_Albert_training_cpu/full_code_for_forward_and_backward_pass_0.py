...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 108624240)) \
        and (___check_obj_id(L['___stack0'], 25869440)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 142543632)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140467049292000))) \
        and (___compile_config_hash() == '8641731555ee8af236a6db4be7f12af5') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140469893195680)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 108624240)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 25869440)) \
        and (___check_type_id(L['cloned_inputs'][0], 87606784)) \
