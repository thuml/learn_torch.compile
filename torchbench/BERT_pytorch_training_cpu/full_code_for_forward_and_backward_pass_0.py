...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 122883792)) \
        and (___check_obj_id(L['___stack0'], 40123616)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 156824528)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139894108201184))) \
        and (___compile_config_hash() == 'b6fecbc31d313f2c756767960bcdc9c9') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(G['__import_torchbenchmark_dot_models_dot_BERT_pytorch_dot_bert_pytorch_dot_model_dot_attention_dot_single'].torch.float16, 139899271411456)) \
...
def __resume_at_20_1(___stack0, self, mod, collect_outputs, cloned_inputs):
    with self.autocast() as __temp_27:
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_37:
        pred = mod(*cloned_inputs)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
