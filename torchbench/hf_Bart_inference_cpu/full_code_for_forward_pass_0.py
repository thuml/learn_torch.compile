...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140454922881392)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 106100944)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 2) \
        and (___check_type_id(L['inputs'][0], 84885536)) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs'][1], 84885536)) \
        and (hasattr(L['inputs'][1], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 23342208)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140452120682720))) \
        and (___compile_config_hash() == 'de4e80a5e11c9e833b994efd358f55d2') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_bart_dot_modeling_bart'].torch.long, 140457283749632)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_bart_dot_modeling_bart'].torch.float16, 140457283749632)) \
