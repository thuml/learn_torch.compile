...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 94707744)) \
        and (___check_obj_id(L['___stack0'], 11946784)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 128665472)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140514734563552))) \
        and (___compile_config_hash() == '8a7f4703973b3f62487d12ab4542f423') \
        and (not ___needs_nopython()) \
