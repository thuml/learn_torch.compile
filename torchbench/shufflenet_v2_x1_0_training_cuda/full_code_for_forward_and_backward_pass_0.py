...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 101058992)) \
        and (___check_obj_id(L['___stack0'], 18299776)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 134973200)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140652816687328))) \
        and (___compile_config_hash() == '5f78aeecec6de2900299bb1961468910') \
        and (not ___needs_nopython()) \
