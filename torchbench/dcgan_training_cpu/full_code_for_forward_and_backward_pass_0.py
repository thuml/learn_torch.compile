...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 112902576)) \
        and (___check_obj_id(L['___stack0'], 30141488)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 146820144)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140561583694048))) \
        and (___compile_config_hash() == 'a5dbab45bcde1d7d7ca2bbd5352f151b') \
