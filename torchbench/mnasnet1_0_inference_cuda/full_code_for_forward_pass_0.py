...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140236924236320)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 107174336)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 1) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 24414912)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140233876594912))) \
        and (___compile_config_hash() == '5c3ab8ad9d2bb0b2e8dcf364c597fded') \
