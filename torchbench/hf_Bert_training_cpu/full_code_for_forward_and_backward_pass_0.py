...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 102183088)) \
        and (___check_obj_id(L['___stack0'], 19426432)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 136097120)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139689518358752))) \
        and (___compile_config_hash() == '455b4f527e7efcf66a15192e5d7caa4b') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139692361534672)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 102183088)) \
        and (___check_type_id(L['cloned_inputs'], 7642176)) \
        and (len(L['cloned_inputs']) == 1) \
        and (___check_obj_id(L['self'].autocast, 19426432)) \
        and (___check_type_id(L['cloned_inputs'][0], 80967696)) \
