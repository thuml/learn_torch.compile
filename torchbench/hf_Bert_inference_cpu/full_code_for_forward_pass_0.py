...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140212049366176)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 121930448)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 1) \
        and (___check_type_id(L['inputs'][0], 100715392)) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 39173248)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140209206337760))) \
        and (___compile_config_hash() == '166ce5ad152901f641ecb9ee8b33d838') \
