...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139919524849520)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 123068960)) \
        and (___check_type_id(L['inputs'], 7642176)) \
        and (len(L['inputs']) == 1) \
        and (___check_type_id(L['inputs'][0], 102049280)) \
        and (hasattr(L['inputs'][0], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['self'].autocast, 40311936)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139916645294304))) \
        and (___compile_config_hash() == '29b8956187f8e5b70bf81a7b0732739a') \
