...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 108274000)) \
        and (___check_obj_id(L['___stack0'], 25513280)) \
        and (hasattr(L['___stack1'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 142212464)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139622078079200))) \
        and (___compile_config_hash() == 'f982e1b409fd920c6f131e4cdc2c132d') \
