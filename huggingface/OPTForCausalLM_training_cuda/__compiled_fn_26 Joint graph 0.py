...
def forward(self, primals, tangents):
    ...
    full: "f32[2047, 50272]" = torch.ops.aten.full.default([2047, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/opt/modeling_opt.py:963, code: shift_logits = logits[..., :-1, :].contiguous()
    full_1: "f32[1, 2047, 50272]" = torch.ops.aten.full.default([1, 2047, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    full_2: "f32[1, 2048, 50272]" = torch.ops.aten.full.default([1, 2048, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
