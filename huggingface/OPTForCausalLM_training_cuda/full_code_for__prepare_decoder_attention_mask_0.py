...
def __guard_0_for__prepare_decoder_attention_mask(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['input_shape'], 140673502535392)) \
        and (len(L['input_shape']) == 2) \
        and (___check_type_id(L['inputs_embeds'], 70973248)) \
        ...
        and (___check_type_id(G['torch'].bool, 140673502529280)) \
        and (G['torch'].bool == torch.bool) \
        and (___check_obj_id(G['_expand_mask'].__defaults__[0], 7628576)) \
        and (___check_type_id(G['_make_causal_mask'].__defaults__[0], 7640416)) \
        and (G['_make_causal_mask'].__defaults__[0] == 0) \
        and (___check_tensors(L['inputs_embeds'], L['attention_mask'], tensor_check_names=tensor_check_names))
...
def __transformed_code_0_for__prepare_decoder_attention_mask(self, attention_mask, input_shape, inputs_embeds, past_key_values_length):
    ...
    return __compiled_fn_6(attention_mask)[0]
