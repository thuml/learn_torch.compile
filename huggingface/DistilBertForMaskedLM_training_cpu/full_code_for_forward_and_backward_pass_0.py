...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140089272958832)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 140131216)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 8846752)) \
        and (___check_type_id(L['self'].grad_scaler, 125847680)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 70706704)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140085994118672))) \
        and (___compile_config_hash() == '8831066c0e477c8e8b3bf1fa1c7fcccc') \
