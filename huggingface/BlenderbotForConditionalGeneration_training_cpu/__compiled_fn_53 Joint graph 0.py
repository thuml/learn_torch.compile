...
def forward(self, primals, tangents):
    primals_1: "f32[8008, 2560]"; primals_2: "f32[1, 8008]"; primals_3: "f32[1, 128, 2560]"; primals_4: "i64[1, 128]"; tangents_1: "f32[]"; tangents_2: "f32[1, 128, 8008]"; 
    ...
    view: "f32[128, 2560]" = torch.ops.aten.view.default(primals_3, [128, 2560]);  primals_3 = None
    mm: "f32[128, 8008]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 128, 8008]" = torch.ops.aten.view.default(mm, [1, 128, 8008]);  mm = None
    add: "f32[1, 128, 8008]" = torch.ops.aten.add.Tensor(view_1, primals_2);  view_1 = primals_2 = None
    ...
    view_2: "f32[128, 8008]" = torch.ops.aten.view.default(add, [-1, 8008])
    view_3: "i64[128]" = torch.ops.aten.view.default(primals_4, [-1]);  primals_4 = None
    amax: "f32[128, 1]" = torch.ops.aten.amax.default(view_2, [1], True)
    sub: "f32[128, 8008]" = torch.ops.aten.sub.Tensor(view_2, amax);  view_2 = amax = None
    exp: "f32[128, 8008]" = torch.ops.aten.exp.default(sub)
    ...
    sub_1: "f32[128, 8008]" = torch.ops.aten.sub.Tensor(sub, log);  sub = log = None
    alias: "f32[128, 8008]" = torch.ops.aten.alias.default(sub_1)
    ...
    full: "f32[128, 8008]" = torch.ops.aten.full.default([128, 8008], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    scatter: "f32[128, 8008]" = torch.ops.aten.scatter.value(full, 1, where_2, -1.0);  full = where_2 = None
    ...
    mul: "f32[128, 8008]" = torch.ops.aten.mul.Tensor(scatter, where_3);  scatter = where_3 = None
    alias_1: "f32[128, 8008]" = torch.ops.aten.alias.default(alias);  alias = None
    exp_1: "f32[128, 8008]" = torch.ops.aten.exp.default(alias_1);  alias_1 = None
    sum_4: "f32[128, 1]" = torch.ops.aten.sum.dim_IntList(mul, [1], True)
    mul_1: "f32[128, 8008]" = torch.ops.aten.mul.Tensor(exp_1, sum_4);  exp_1 = sum_4 = None
    sub_2: "f32[128, 8008]" = torch.ops.aten.sub.Tensor(mul, mul_1);  mul = mul_1 = None
    view_4: "f32[1, 128, 8008]" = torch.ops.aten.view.default(sub_2, [1, 128, 8008]);  sub_2 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/blenderbot/modeling_blenderbot.py:1363, code: masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
    add_1: "f32[1, 128, 8008]" = torch.ops.aten.add.Tensor(tangents_2, view_4);  tangents_2 = view_4 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/blenderbot/modeling_blenderbot.py:1358, code: lm_logits = self.lm_head(outputs[0]) + self.final_logits_bias
    view_5: "f32[128, 8008]" = torch.ops.aten.view.default(add_1, [128, 8008]);  add_1 = None
    permute_1: "f32[8008, 128]" = torch.ops.aten.permute.default(view_5, [1, 0])
    mm_1: "f32[8008, 2560]" = torch.ops.aten.mm.default(permute_1, view);  permute_1 = view = None
    permute_2: "f32[2560, 8008]" = torch.ops.aten.permute.default(mm_1, [1, 0]);  mm_1 = None
    permute_3: "f32[8008, 2560]" = torch.ops.aten.permute.default(permute, [1, 0]);  permute = None
    mm_2: "f32[128, 2560]" = torch.ops.aten.mm.default(view_5, permute_3);  view_5 = permute_3 = None
    view_6: "f32[1, 128, 2560]" = torch.ops.aten.view.default(mm_2, [1, 128, 2560]);  mm_2 = None
    permute_4: "f32[8008, 2560]" = torch.ops.aten.permute.default(permute_2, [1, 0]);  permute_2 = None
