...
def __guard_5_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140276879850176)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 158302368)) \
        and (___check_type_id(L['input_ids_shape'], 140278872821472)) \
        ...
        and (___check_type_id(G['torch'].long, 140278872815360)) \
...
def __guard_2_for_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 140276879848256)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 158302368)) \
        and (___check_type_id(L['input_ids_shape'], 140278872821472)) \
        and (len(L['input_ids_shape']) == 2) \
...
def transformed_forward(self, input_ids_shape, past_key_values_length):
    ...
    if __guard_5_for_forward(L):
        return __transformed_code_5_for_forward(self, input_ids_shape, past_key_values_length)
