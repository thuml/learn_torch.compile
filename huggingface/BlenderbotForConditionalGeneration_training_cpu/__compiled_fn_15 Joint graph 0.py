...
def forward(self, primals, tangents):
    primals_1: "f32[128, 2560]"; tangents_1: "f32[128, 2560]"; 
    ...
    where: "f32[128, 2560]" = torch.ops.aten.where.self(unsqueeze, scalar_tensor, tangents_1);  unsqueeze = scalar_tensor = tangents_1 = None
    full: "f32[128, 2560]" = torch.ops.aten.full.default([128, 2560], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    _unsafe_index_put: "f32[128, 2560]" = torch.ops.aten._unsafe_index_put.default(full, [iota], where, True);  full = iota = where = None
