        ...
        }
        {
            ...
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(128L); x0+=static_cast<long>(1L))
            {
                for(long x1=static_cast<long>(0L); x1<static_cast<long>(8008L); x1+=static_cast<long>(8L))
                {
                    ...
                    auto tmp7 = tmp3 - tmp6;
                    tmp7.store(out_ptr2 + static_cast<long>(x1 + (8008L*x0)));
                }
            }
        }
        #pragma omp single
            ...
            {
                {
                    ...
                    for(long x0=static_cast<long>(0L); x0<static_cast<long>(128L); x0+=static_cast<long>(1L))
                    {
                        ...
                        auto tmp6 = decltype(tmp5)(tmp5 + 8008);
                        auto tmp7 = tmp5 < 0;
                        auto tmp8 = tmp7 ? tmp6 : tmp5;
                        TORCH_CHECK((0 <= tmp8) & (tmp8 < 8008L), "index out of bounds: 0 <= tmp8 < 8008L")
                        auto tmp9 = out_ptr2[static_cast<long>(tmp8 + (8008L*x0))];
...
def call(args):
    ...
    args.clear()
    assert_size_stride(primals_1, (8008, 2560), (2560, 1))
    assert_size_stride(primals_2, (1, 8008), (8008, 1))
    assert_size_stride(primals_3, (1, 128, 2560), (327680, 2560, 1))
    ...
    extern_kernels.mm(reinterpret_tensor(primals_3, (128, 2560), (2560, 1), 0), reinterpret_tensor(primals_1, (2560, 8008), (1, 2560), 0), out=buf0)
    buf1 = reinterpret_tensor(buf0, (1, 128, 8008), (1025024, 8008, 1), 0); del buf0  # reuse
    ...
    buf4 = empty((128, 8008), device='cpu', dtype=torch.float32)
    ...
    del buf5
    del primals_2
    return (buf8, buf1, primals_4, reinterpret_tensor(primals_3, (128, 2560), (2560, 1), 0), buf4, buf6, reinterpret_tensor(primals_1, (8008, 2560), (2560, 1), 0), )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_1 = rand_strided((8008, 2560), (2560, 1), device='cpu', dtype=torch.float32)
    primals_2 = rand_strided((1, 8008), (8008, 1), device='cpu', dtype=torch.float32)
    primals_3 = rand_strided((1, 128, 2560), (327680, 2560, 1), device='cpu', dtype=torch.float32)
