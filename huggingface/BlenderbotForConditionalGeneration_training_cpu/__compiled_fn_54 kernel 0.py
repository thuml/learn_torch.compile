                       ...
                       long* out_ptr1)
{
    #pragma omp parallel num_threads(28)
    {
        {
            #pragma omp for 
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(1025024L); x0+=static_cast<long>(8L))
            {
    ...
    #pragma omp parallel num_threads(28)
    {
            ...
            {
                {
                    ...
                    {
                        auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<long>(x1 + (8008L*x0)));
        ...
        }
        {
            ...
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(128L); x0+=static_cast<long>(1L))
            {
                for(long x1=static_cast<long>(0L); x1<static_cast<long>(8008L); x1+=static_cast<long>(8L))
                {
                    auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr4 + static_cast<long>(x1 + (8008L*x0)));
                    auto tmp1 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<long>(x1 + (8008L*x0)));
                    auto tmp2 = in_ptr1[static_cast<long>(x0)];
                    auto tmp5 = in_ptr2[static_cast<long>(0L)];
                    auto tmp6 = in_ptr3[static_cast<long>(0L)];
                    auto tmp12 = at::vec::Vectorized<float>::loadu(in_ptr5 + static_cast<long>(x1 + (8008L*x0)));
                    ...
                    tmp18.store(in_out_ptr0 + static_cast<long>(x1 + (8008L*x0)));
                }
            }
        }
    }
...
def call(args):
    ...
    assert_size_stride(sub_1, (128, 8008), (8008, 1))
    assert_size_stride(convert_element_type, (), ())
    assert_size_stride(permute_3, (8008, 2560), (2560, 1))
    assert_size_stride(tangents_1, (), ())
    assert_size_stride(tangents_2, (1, 128, 8008), (1025024, 8008, 1))
    buf0 = empty((128, 8008), device='cpu', dtype=torch.float32)
    ...
    buf3 = empty((128, 8008), device='cpu', dtype=torch.float32)
    buf5 = reinterpret_tensor(buf3, (1, 128, 8008), (1025024, 8008, 1), 0); del buf3  # reuse
    ...
    del tangents_1
    del tangents_2
    buf6 = empty((8008, 2560), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (8008, 128), (1, 8008), 0), view, out=buf6)
    del view
    buf7 = empty((128, 2560), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (128, 8008), (8008, 1), 0), permute_3, out=buf7)
    del buf5
    del permute_3
    return (reinterpret_tensor(buf6, (8008, 2560), (2560, 1), 0), None, reinterpret_tensor(buf7, (1, 128, 2560), (327680, 2560, 1), 0), None, )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_4 = rand_strided((1, 128), (128, 1), device='cpu', dtype=torch.int64)
    view = rand_strided((128, 2560), (2560, 1), device='cpu', dtype=torch.float32)
    sub_1 = rand_strided((128, 8008), (8008, 1), device='cpu', dtype=torch.float32)
    convert_element_type = rand_strided((), (), device='cpu', dtype=torch.float32)
    permute_3 = rand_strided((8008, 2560), (2560, 1), device='cpu', dtype=torch.float32)
    tangents_1 = rand_strided((), (), device='cpu', dtype=torch.float32)
    tangents_2 = rand_strided((1, 128, 8008), (1025024, 8008, 1), device='cpu', dtype=torch.float32)
