...
def __resume_at_144_54(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    'Failed to decompile.'

def transformed___resume_at_144_54(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_144_54(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_144_54 ============#

def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 160387344)) \
        and (___check_obj_id(L['___stack0'], 29097392)) \
        and (___check_type_id(L['___stack1'], 147026016)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 146122048)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140276880323920)) \
        ...
        and (___check_obj_id(L['self'].autocast, 29097392)) \
...
def __resume_at_20_1(___stack0, self, mod, collect_outputs, cloned_inputs):
    with self.autocast() as __temp_110:
        __temp_112 = {}
        __temp_112.update(cloned_inputs)
        pred = mod(*(), **__temp_112)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_135:
        __temp_137 = {}
        __temp_137.update(cloned_inputs)
        pred = mod(*(), **__temp_137)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
