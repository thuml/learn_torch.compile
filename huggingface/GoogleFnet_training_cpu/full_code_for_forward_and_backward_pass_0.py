...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139762339446304)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 163726016)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 32447616)) \
        and (___check_type_id(L['self'].grad_scaler, 149485328)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 94307984)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139759061687824))) \
        and (___compile_config_hash() == '54411666562e0baece4230de3da72c64') \
