...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 172228608)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140565878090688)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 172228608)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'decoder_input_ids', 'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 40938176)) \
        and (___check_type_id(L['self'].grad_scaler, 157966800)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.long, 140567870494464)) \
        and (G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_torch_dot_nn_dot_modules_dot_module']._global_backward_hooks, 7489504)) \
        and (set(G['__import_torch_dot_nn_dot_modules_dot_module']._global_backward_hooks.keys()) == set()) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float16, 140567870494464)) \
        and (G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float32, 140567870494464)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.bfloat16, 140567870494464)) \
...
def __guard_0_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['___stack0'].keys()) == {'decoder_input_ids', 'labels', 'input_ids'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140562609102352))) \
        and (___compile_config_hash() == '14a0b0a43bdc0a6fe55cf3c4841f2342') \
        and (not ___needs_nopython())
...
def __guard_0_for_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['inputs'].keys()) == {'decoder_input_ids', 'labels', 'input_ids'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140562609102352))) \
        and (___compile_config_hash() == '14a0b0a43bdc0a6fe55cf3c4841f2342') \
        and (not ___needs_nopython())
