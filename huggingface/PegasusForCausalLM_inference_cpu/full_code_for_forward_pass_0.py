...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139761780143488)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 167835024)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 36543888)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139758757322256))) \
        and (___compile_config_hash() == 'b80af920d16cf04fe2400d5239c8eaba') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_pegasus_dot_modeling_pegasus'].torch.long, 139764018775808)) \
