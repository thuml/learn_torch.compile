...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139963652127328)) \
        ...
        and (___check_type_id(L['transformer_outputs'], 139284496)) \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139963652127328)) \
        and (L['self'].training == True) \
        and (___check_type_id(L['labels'], 83334832)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 83334832)) \
        ...
        and (___check_type_id(G['torch'].long, 139965635950336)) \
        and (G['torch'].long == torch.int64) \
        and (___check_type_id(G['torch'].float32, 139965635950336)) \
