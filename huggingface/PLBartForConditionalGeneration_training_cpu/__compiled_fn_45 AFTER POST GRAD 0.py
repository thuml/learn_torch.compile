...
def forward(self, primals_1: "f32[50005, 768]", primals_2: "f32[1, 50005]", primals_3: "f32[1, 1024, 768]", primals_4: "i64[1, 1024]"):
    ...
    view: "f32[1024, 768]" = torch.ops.aten.reshape.default(primals_3, [1024, 768]);  primals_3 = None
    mm: "f32[1024, 50005]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 1024, 50005]" = torch.ops.aten.reshape.default(mm, [1, 1024, 50005]);  mm = None
    ...
    view_2: "f32[1024, 50005]" = torch.ops.aten.reshape.default(add, [-1, 50005])
