...
def __guard_9_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139631876482640)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 151550672)) \
        and (___check_type_id(L['input_ids'], 81622544)) \
        ...
        and (___check_type_id(G['torch'].long, 139633991722752)) \
...
def __guard_2_for_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 139631876374144)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 151550672)) \
        and (___check_type_id(L['input_ids'], 81622544)) \
        and (hasattr(L['input_ids'], '_dynamo_dynamic_indices') == False) \
