...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 161895968)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140192150101520))) \
        and (___compile_config_hash() == 'b68e046b1d68de300aebd6e1894adce6') \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140195420155920)) \
        ...
        and (___check_obj_id(L['self'].autocast, 30608384)) \
        and (___check_type_id(L['self'].grad_scaler, 147609520)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 92469296)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.long, 140197411866368)) \
        and (G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.float, 140197411866368)) \
