...
def forward(self, primals, tangents):
    primals_1: "f32[50265, 1024]"; primals_2: "f32[1, 50265]"; primals_3: "f32[1, 1024, 1024]"; primals_4: "i64[1, 1024]"; tangents_1: "f32[]"; tangents_2: "f32[1, 1024, 50265]"; 
    ...
    view: "f32[1024, 1024]" = torch.ops.aten.view.default(primals_3, [1024, 1024]);  primals_3 = None
    mm: "f32[1024, 50265]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 1024, 50265]" = torch.ops.aten.view.default(mm, [1, 1024, 50265]);  mm = None
    add: "f32[1, 1024, 50265]" = torch.ops.aten.add.Tensor(view_1, primals_2);  view_1 = primals_2 = None
    ...
    view_2: "f32[1024, 50265]" = torch.ops.aten.view.default(add, [-1, 50265])
    ...
    sub: "f32[1024, 50265]" = torch.ops.aten.sub.Tensor(view_2, amax);  view_2 = amax = None
    exp: "f32[1024, 50265]" = torch.ops.aten.exp.default(sub)
    ...
    sub_1: "f32[1024, 50265]" = torch.ops.aten.sub.Tensor(sub, log);  sub = log = None
    alias: "f32[1024, 50265]" = torch.ops.aten.alias.default(sub_1)
    ...
    full: "f32[1024, 50265]" = torch.ops.aten.full.default([1024, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    mul: "f32[1024, 50265]" = torch.ops.aten.mul.Tensor(scatter, where_3);  scatter = where_3 = None
    ...
    mul_1: "f32[1024, 50265]" = torch.ops.aten.mul.Tensor(exp_1, sum_4);  exp_1 = sum_4 = None
    sub_2: "f32[1024, 50265]" = torch.ops.aten.sub.Tensor(mul, mul_1);  mul = mul_1 = None
    view_4: "f32[1, 1024, 50265]" = torch.ops.aten.view.default(sub_2, [1, 1024, 50265]);  sub_2 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/mbart/modeling_mbart.py:1379, code: masked_lm_loss = loss_fct(lm_logits.view(-1, self.config.vocab_size), labels.view(-1))
    add_1: "f32[1, 1024, 50265]" = torch.ops.aten.add.Tensor(tangents_2, view_4);  tangents_2 = view_4 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/mbart/modeling_mbart.py:1374, code: lm_logits = self.lm_head(outputs[0]) + self.final_logits_bias
    view_5: "f32[1024, 50265]" = torch.ops.aten.view.default(add_1, [1024, 50265]);  add_1 = None
    permute_1: "f32[50265, 1024]" = torch.ops.aten.permute.default(view_5, [1, 0])
    ...
    mm_2: "f32[1024, 1024]" = torch.ops.aten.mm.default(view_5, permute_3);  view_5 = permute_3 = None
    view_6: "f32[1, 1024, 1024]" = torch.ops.aten.view.default(mm_2, [1, 1024, 1024]);  mm_2 = None
