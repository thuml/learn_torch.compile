...
def forward(self, primals_4: "i64[1, 1024]", view: "f32[1024, 1024]", sub_1: "f32[1024, 50265]", convert_element_type: "f32[]", permute_3: "f32[50265, 1024]", tangents_1: "f32[]", tangents_2: "f32[1, 1024, 50265]"):
    ...
    view_3: "i64[1024]" = torch.ops.aten.reshape.default(primals_4, [-1]);  primals_4 = None
    ...
    full_default_3: "f32[1024, 50265]" = torch.ops.aten.full.default([1024, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    exp_1: "f32[1024, 50265]" = torch.ops.aten.exp.default(sub_1);  sub_1 = None
    ...
    view_4: "f32[1, 1024, 50265]" = torch.ops.aten.reshape.default(sub_2, [1, 1024, 50265]);  sub_2 = None
    ...
    view_5: "f32[1024, 50265]" = torch.ops.aten.reshape.default(add_1, [1024, 50265]);  add_1 = None
    ...
    view_6: "f32[1, 1024, 1024]" = torch.ops.aten.reshape.default(mm_2, [1, 1024, 1024]);  mm_2 = None
