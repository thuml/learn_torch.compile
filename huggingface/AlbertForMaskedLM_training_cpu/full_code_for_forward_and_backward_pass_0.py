...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139766062790256)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 148011328)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 16723264)) \
        and (___check_type_id(L['self'].grad_scaler, 133728576)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 78583888)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139766063603216))) \
        and (___compile_config_hash() == '1bd808bd568b5c0576233e67ebb6fad9') \
