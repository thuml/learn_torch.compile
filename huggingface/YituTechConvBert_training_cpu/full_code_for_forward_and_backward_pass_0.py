...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140091832268640)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 164438592)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 33148224)) \
        and (___check_type_id(L['self'].grad_scaler, 150189664)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 95008208)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140088555953680))) \
        and (___compile_config_hash() == '9db7829a53a26ce3471dd8d60fd91027') \
        and (not ___needs_nopython()) \
        and (___check_type_id(G['__import_transformers_dot_activations'].ACT2FN, 150938416)) \
