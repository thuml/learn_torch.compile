...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140717728776688)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 164776416)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 33487840)) \
        and (___check_type_id(L['self'].grad_scaler, 150481632)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 95349456)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140714436599312))) \
        and (___compile_config_hash() == 'a57b51d2e9ef9c0b692024cd51a404c8') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mobilebert_dot_modeling_mobilebert'].torch.long, 140719698126592)) \
