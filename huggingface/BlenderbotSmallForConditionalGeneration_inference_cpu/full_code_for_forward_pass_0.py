...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140334932723120)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 162800768)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'labels', 'decoder_input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 31513104)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140331659288080))) \
        and (___compile_config_hash() == '3efa1df0f42b542eb369cde7d1fc7aa8') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.long, 140336921126656)) \
        and (G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.float16, 140336921126656)) \
