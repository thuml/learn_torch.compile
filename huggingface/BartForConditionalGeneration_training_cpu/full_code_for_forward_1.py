...
def __resume_at_278_24(___stack0, return_dict, encoder_outputs):
    'Failed to decompile.'

def transformed___resume_at_278_24(___stack0, return_dict, encoder_outputs):
    L = {"___stack0": ___stack0, "return_dict": return_dict, "encoder_outputs": encoder_outputs}
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_278_24(___stack0, return_dict, encoder_outputs)

#============ end of __resume_at_278_24 ============#

def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 140532045397920)) \
        and (L['self'].training == True) \
        and (___check_type_id(L['___stack0'], 158750224)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140528780320272))) \
        and (___compile_config_hash() == '7584c6bd637f099ceb840e02ba6e1acc') \
...
def __transformed_code_0_for_resume_in_forward(___stack0, self, attention_mask, decoder_input_ids, decoder_attention_mask, decoder_head_mask, cross_attn_head_mask, past_key_values, decoder_inputs_embeds, use_cache, output_attentions, output_hidden_states, return_dict):
    ...
    return __resume_at_278_24(self.decoder(input_ids=decoder_input_ids,
...
# Note: if there is a transformed version below, this function might well not be executed directly. Please check the transformed version if possible.
def __resume_at_162_7(___stack0, self, attention_mask, decoder_input_ids, decoder_attention_mask, decoder_head_mask, cross_attn_head_mask, past_key_values, decoder_inputs_embeds, use_cache, output_attentions, output_hidden_states, return_dict):
    'Failed to decompile.'

def transformed___resume_at_162_7(___stack0, self, attention_mask, decoder_input_ids, decoder_attention_mask, decoder_head_mask, cross_attn_head_mask, past_key_values, decoder_inputs_embeds, use_cache, output_attentions, output_hidden_states, return_dict):
    ...
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_162_7(___stack0, self, attention_mask, decoder_input_ids, decoder_attention_mask, decoder_head_mask, cross_attn_head_mask, past_key_values, decoder_inputs_embeds, use_cache, output_attentions, output_hidden_states, return_dict)

#============ end of __resume_at_162_7 ============#
...
def __guard_1_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___compile_config_hash() == '7584c6bd637f099ceb840e02ba6e1acc') \
        and (not ___needs_nopython()) \
        and (___check_tensors(L['input_ids'], L['decoder_input_ids'], tensor_check_names=tensor_check_names))
