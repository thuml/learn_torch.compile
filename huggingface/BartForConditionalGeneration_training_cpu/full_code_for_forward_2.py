...
def __guard_15_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140532044687264)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 172627568)) \
        and (___check_type_id(L['input_ids'], 102673952)) \
        ...
        and (___check_type_id(G['torch'].long, 140534041851648)) \
...
def __helper_outer_function():
    ...
    __class__ = None
    def __transformed_code_15_for_forward(self, input_ids, past_key_values_length):
        ...
        nonlocal __class__
        return __compiled_fn_26()[0]


def __guard_2_for_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 140532044691872)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 172627568)) \
        and (___check_type_id(L['input_ids'], 102673952)) \
        and (hasattr(L['input_ids'], '_dynamo_dynamic_indices') == False) \
...
def transformed_forward(self, input_ids, past_key_values_length):
    ...
    if __guard_15_for_forward(L):
        return __transformed_code_15_for_forward(self, input_ids, past_key_values_length)
