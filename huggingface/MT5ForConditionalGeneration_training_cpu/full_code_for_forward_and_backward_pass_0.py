...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140490680910464)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 155974608)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels', 'decoder_input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 24690112)) \
        and (___check_type_id(L['self'].grad_scaler, 141689392)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140487412866576))) \
        and (___compile_config_hash() == 'de63b383308b247b2e853b410b226097') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.long, 140492674316032)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float16, 140492674316032)) \
        and (G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float32, 140492674316032)) \
        and (G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float32 == torch.float32) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.bfloat16, 140492674316032)) \
