...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139671295340992)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 155275664)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 23997952)) \
        and (___check_type_id(L['self'].grad_scaler, 140990016)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 85857744)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139668023090704))) \
        and (___compile_config_hash() == 'c7934a68fa237284ed7eeb79afae7ab7') \
        ...
        and (G['__import_transformers_dot_modeling_utils'].ENV_VARS_TRUE_VALUES == {'TRUE', 'YES', '1', 'ON'}) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_megatron_bert_dot_modeling_megatron_bert'].torch.long, 139673284536064)) \
