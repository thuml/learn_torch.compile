...
# kernel path: /tmp/torchinductor_youkaichao/jj/cjj3lnmu3uwrl6gkgd2wsuqb7berawkpth4vdhymc4qixghsy2fw.py
...
# kernel path: /tmp/torchinductor_youkaichao/u4/cu44abxkvfelnzgqd36ohg6273jqbgnns6rkewomrh4iba7g2plh.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => amax
...
# kernel path: /tmp/torchinductor_youkaichao/mi/cmiawgmvyev3mvgvi7ek54imx75kolby3ywqg67jcs5aeslt5pdu.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => exp, sub, sum_1
...
# kernel path: /tmp/torchinductor_youkaichao/3r/c3ry3ofqlqrgla7bjclybrltyndwudzu6iz27vowir3hhzkz3dl7.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => exp, sub, sum_1
...
# kernel path: /tmp/torchinductor_youkaichao/ob/cobdu46xn2ji7ty2r5teh7uqdpkvzv5zqfaw46u5kazuw2ghxxys.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => log, sub, sub_1
...
# kernel path: /tmp/torchinductor_youkaichao/qb/cqber4qkrmimw6bbepxslvdpl7pku6twgmxdmhs5o4kqi2etd4st.py
...
triton_per_fused_nll_loss_forward_5 = async_compile.triton('triton_', '''
import triton
import triton.language as tl
from torch._inductor.ir import ReductionHint
from torch._inductor.ir import TileHint
...
@persistent_reduction(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_per_fused_nll_loss_forward_5', 'mutated_arg_names': ['in_out_ptr0']}
)
@triton.jit
...
def call(args):
    ...
    args.clear()
    assert_size_stride(primals_1, (50265, 512), (512, 1))
    assert_size_stride(primals_2, (1, 128, 512), (65536, 512, 1))
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        extern_kernels.mm(reinterpret_tensor(primals_2, (128, 512), (512, 1), 0), reinterpret_tensor(primals_1, (512, 50265), (1, 512), 0), out=buf0)
        buf1 = empty_strided((128, 1, 7), (7, 896, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        stream0 = get_cuda_stream(0)
        triton_red_fused__log_softmax_0.run(buf0, buf1, 896, 7181, grid=grid(896), stream=stream0)
        buf2 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_per_fused__log_softmax_1.run(buf1, buf2, 128, 7, grid=grid(128), stream=stream0)
        buf3 = buf1; del buf1  # reuse
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_red_fused__log_softmax_2.run(buf0, buf2, buf3, 896, 7181, grid=grid(896), stream=stream0)
        buf4 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_per_fused__log_softmax_3.run(buf3, buf4, 128, 7, grid=grid(128), stream=stream0)
        del buf3
        buf5 = empty((128, 50265), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_poi_fused__log_softmax_4.run(buf0, buf2, buf4, buf5, 6433920, grid=grid(6433920), stream=stream0)
        del buf2
        del buf4
        buf8 = empty((), device='cuda', dtype=torch.float32)
        buf7 = empty((), device='cuda', dtype=torch.float32)
        buf9 = buf8; del buf8  # reuse
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_forward]
        triton_per_fused_nll_loss_forward_5.run(buf9, primals_3, buf5, buf7, 1, 128, grid=grid(1), stream=stream0)
        return (buf9, reinterpret_tensor(buf0, (1, 128, 50265), (6433920, 50265, 1), 0), primals_3, reinterpret_tensor(primals_2, (128, 512), (512, 1), 0), buf5, buf7, reinterpret_tensor(primals_1, (50265, 512), (512, 1), 0), )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_1 = rand_strided((50265, 512), (512, 1), device='cuda:0', dtype=torch.float32)
    primals_2 = rand_strided((1, 128, 512), (65536, 512, 1), device='cuda:0', dtype=torch.float32)
