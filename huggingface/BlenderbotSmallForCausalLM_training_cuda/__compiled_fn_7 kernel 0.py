...
# kernel path: /tmp/torchinductor_youkaichao/ob/cobzxuipy6g6u4htrqlgxyba4w4kz4ec7in27oz5ak64z3z6gk62.py
# Source Nodes: [positions], Original ATen: [aten.arange]
# positions => iota
triton_poi_fused_arange_0 = async_compile.triton('triton_', '''
import triton
...
@pointwise(
    ...
    inductor_meta={'autotune_hints': set(), 'kernel_name': 'triton_poi_fused_arange_0', 'mutated_arg_names': []},
...
# kernel path: /tmp/torchinductor_youkaichao/q2/cq2q567wt7tifbqvnncpfos6ar6klngeyww3xt3xqdpycfk3jwkc.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf0 = empty((128, ), device='cuda', dtype=torch.int64)
        # Source Nodes: [positions], Original ATen: [aten.arange]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_arange_0.run(buf0, 128, grid=grid(128), stream=stream0)
        buf1 = empty((128, 512), device='cuda', dtype=torch.float32)
        # Source Nodes: [embedding], Original ATen: [aten.embedding]
        triton_poi_fused_embedding_1.run(primals_1, buf1, 65536, grid=grid(65536), stream=stream0)
        del primals_1
        return (buf1, buf0, )
