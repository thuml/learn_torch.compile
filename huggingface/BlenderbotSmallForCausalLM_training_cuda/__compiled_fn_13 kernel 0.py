...
# kernel path: /tmp/torchinductor_youkaichao/th/cthaxgrx2uaafvteujo7ixm6bnlkvx4a6ekggncshs2lq5a6ek5g.py
...
# kernel path: /tmp/torchinductor_youkaichao/4g/c4gyzo5ijgpxcveck2yg2donbifveisppdmzsnqx3je5duoqlsis.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf2 = empty((1, 16, 128, 32), device='cuda', dtype=torch.float32)
        # Source Nodes: [key_states], Original ATen: [aten.clone]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_clone_0.run(buf1, primals_4, buf2, 65536, grid=grid(65536), stream=stream0)
        del primals_4
        buf3 = buf1; del buf1  # reuse
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_17, (128, 512), (512, 1), 0), reinterpret_tensor(primals_5, (512, 512), (1, 512), 0), out=buf3)
        buf4 = empty((1, 16, 128, 32), device='cuda', dtype=torch.float32)
        # Source Nodes: [value_states], Original ATen: [aten.clone]
        triton_poi_fused_clone_0.run(buf3, primals_6, buf4, 65536, grid=grid(65536), stream=stream0)
        del primals_6
        buf5 = reinterpret_tensor(buf3, (1, 16, 128, 32), (65536, 4096, 32, 1), 0); del buf3  # reuse
        # Source Nodes: [contiguous_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_1.run(buf0, primals_2, buf5, 65536, grid=grid(65536), stream=stream0)
        del primals_2
        ...
        buf34 = empty((16, 128, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [attn_probs, attn_weights_3], Original ATen: [aten._softmax, aten.clone, aten.detach]
        triton_per_fused__softmax_clone_detach_2.run(buf6, primals_18, buf9, buf34, 2048, 128, grid=grid(2048), stream=stream0)
        del primals_18
        buf10 = reinterpret_tensor(buf0, (16, 128, 32), (4096, 32, 1), 0); del buf0  # reuse
        ...
        buf21 = reinterpret_tensor(buf6, (128, 2048), (2048, 1), 0); del buf6  # reuse
