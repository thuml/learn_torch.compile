...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139973860720208)) \
        and (L['self'].training == True) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['___stack0'], 149959184)) \
        ...
        and (len(L['___stack0'].past_key_values) == 8) \
...
def __transformed_code_0_for_resume_in_forward(___stack0, self, labels, return_dict):
    ...
    graph_out_0 = __compiled_fn_22(___stack0.last_hidden_state, labels)
    import importlib
