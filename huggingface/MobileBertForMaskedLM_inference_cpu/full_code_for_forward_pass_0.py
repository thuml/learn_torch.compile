...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140193042807152)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 152438048)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 21155312)) \
        and (hasattr(L['inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs']['input_ids'], 83015312)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140189896744464))) \
        and (___compile_config_hash() == '56def1eed6040f651c30633ca39121b0') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mobilebert_dot_modeling_mobilebert'].torch.long, 140195158206208)) \
