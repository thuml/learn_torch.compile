...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139622182484480)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 167559872)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'end_positions', 'start_positions'}) \
        and (___check_obj_id(L['self'].autocast, 36273600)) \
        and (___check_type_id(L['self'].grad_scaler, 153266272)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 98135584)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139622183427600))) \
        and (___compile_config_hash() == '9cdb1489ce19dbc710820c469a7ae144') \
