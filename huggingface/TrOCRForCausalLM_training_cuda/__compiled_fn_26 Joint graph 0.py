...
def forward(self, primals, tangents):
    primals_1: "f32[50265, 1024]"; primals_2: "f32[1, 256, 1024]"; primals_3: "i64[1, 256]"; tangents_1: "f32[]"; tangents_2: "f32[1, 256, 50265]"; 
    ...
    view: "f32[256, 1024]" = torch.ops.aten.view.default(primals_2, [256, 1024]);  primals_2 = None
    mm: "f32[256, 50265]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 256, 50265]" = torch.ops.aten.view.default(mm, [1, 256, 50265]);  mm = None
    ...
    view_2: "f32[256, 50265]" = torch.ops.aten.view.default(view_1, [-1, 50265])
    view_3: "i64[256]" = torch.ops.aten.view.default(primals_3, [-1]);  primals_3 = None
    amax: "f32[256, 1]" = torch.ops.aten.amax.default(view_2, [1], True)
    sub: "f32[256, 50265]" = torch.ops.aten.sub.Tensor(view_2, amax);  view_2 = amax = None
    exp: "f32[256, 50265]" = torch.ops.aten.exp.default(sub)
    ...
    sub_1: "f32[256, 50265]" = torch.ops.aten.sub.Tensor(sub, log);  sub = log = None
    alias: "f32[256, 50265]" = torch.ops.aten.alias.default(sub_1)
    ne: "b8[256]" = torch.ops.aten.ne.Scalar(view_3, -100)
    ...
    where: "i64[256]" = torch.ops.aten.where.self(ne, view_3, scalar_tensor);  ne = scalar_tensor = None
    unsqueeze: "i64[256, 1]" = torch.ops.aten.unsqueeze.default(where, 1);  where = None
    gather: "f32[256, 1]" = torch.ops.aten.gather.default(sub_1, 1, unsqueeze);  sub_1 = unsqueeze = None
    ...
    ne_1: "b8[256]" = torch.ops.aten.ne.Scalar(view_3, -100)
    ...
    where_1: "f32[256]" = torch.ops.aten.where.self(ne_1, neg, scalar_tensor_1);  ne_1 = neg = scalar_tensor_1 = None
    ne_2: "b8[256]" = torch.ops.aten.ne.Scalar(view_3, -100)
    ...
    where_2: "i64[256, 1]" = torch.ops.aten.where.self(ne_3, unsqueeze_1, scalar_tensor_2);  ne_3 = scalar_tensor_2 = None
    full: "f32[256, 50265]" = torch.ops.aten.full.default([256, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    scatter: "f32[256, 50265]" = torch.ops.aten.scatter.value(full, 1, where_2, -1.0);  full = where_2 = None
    ne_4: "b8[256, 1]" = torch.ops.aten.ne.Scalar(unsqueeze_1, -100);  unsqueeze_1 = None
    ...
    where_3: "f32[256, 1]" = torch.ops.aten.where.self(ne_4, div_1, scalar_tensor_3);  ne_4 = div_1 = scalar_tensor_3 = None
    mul: "f32[256, 50265]" = torch.ops.aten.mul.Tensor(scatter, where_3);  scatter = where_3 = None
    alias_1: "f32[256, 50265]" = torch.ops.aten.alias.default(alias);  alias = None
    exp_1: "f32[256, 50265]" = torch.ops.aten.exp.default(alias_1);  alias_1 = None
    ...
    view_4: "f32[1, 256, 50265]" = torch.ops.aten.view.default(sub_2, [1, 256, 50265]);  sub_2 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/trocr/modeling_trocr.py:983, code: loss = loss_fct(logits.view(-1, self.config.vocab_size), labels.view(-1))
    add: "f32[1, 256, 50265]" = torch.ops.aten.add.Tensor(tangents_2, view_4);  tangents_2 = view_4 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/trocr/modeling_trocr.py:978, code: logits = self.output_projection(outputs[0])
    view_5: "f32[256, 50265]" = torch.ops.aten.view.default(add, [256, 50265]);  add = None
    ...
    view_6: "f32[1, 256, 1024]" = torch.ops.aten.view.default(mm_2, [1, 256, 1024]);  mm_2 = None
