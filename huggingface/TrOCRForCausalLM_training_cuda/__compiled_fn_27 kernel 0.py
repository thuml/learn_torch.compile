...
# kernel path: /tmp/torchinductor_youkaichao/q2/cq2vwexu76syb2bkd6ore27gs5moazxvi36hozzjpysakz7yzjus.py
...
# kernel path: /tmp/torchinductor_youkaichao/ma/cmav5kxc5hzckbcly3n2f6nrg4aytbyucr2idpuzyvnhhxbgxwik.py
# Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
# loss => full_default
...
# kernel path: /tmp/torchinductor_youkaichao/vy/cvy7ptfe7v224rpc52wqym4gowbgvjhtwxutj7fna5k3ekdks6f3.py
...
def call(args):
    ...
    args.clear()
    assert_size_stride(primals_3, (1, 256), (256, 1))
    assert_size_stride(view, (256, 1024), (1024, 1))
    assert_size_stride(sub_1, (256, 50265), (50265, 1))
    assert_size_stride(convert_element_type, (), ())
    assert_size_stride(permute_3, (50265, 1024), (1024, 1))
    assert_size_stride(tangents_1, (), ())
    assert_size_stride(tangents_2, (1, 256, 50265), (12867840, 50265, 1))
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0) # no-op to ensure context
        buf0 = empty((256, 50265), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_nll_loss_backward_nll_loss_forward_0.run(buf0, 12867840, grid=grid(12867840), stream=stream0)
        buf1 = empty_strided((256, 1), (1, 256), device='cuda', dtype=torch.int64)
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
        triton_poi_fused_nll_loss_backward_nll_loss_forward_1.run(primals_3, buf1, 256, grid=grid(256), stream=stream0)
        aten.scatter_(buf0,1,buf1,-1.0)
        del buf1
        buf5 = empty((1, 256, 50265), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax_backward_data, aten.add, aten.nll_loss_backward, aten.nll_loss_forward]
        triton_red_fused__log_softmax_backward_data_add_nll_loss_backward_nll_loss_forward_2.run(buf0, primals_3, tangents_1, convert_element_type, tangents_2, sub_1, buf5, 256, 50265, grid=grid(256), stream=stream0)
        del buf0
        ...
        del tangents_1
        del tangents_2
        buf6 = empty((50265, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (50265, 256), (1, 50265), 0), view, out=buf6)
        del view
        buf7 = empty((256, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (256, 50265), (50265, 1), 0), permute_3, out=buf7)
        del buf5
        del permute_3
        return (reinterpret_tensor(buf6, (50265, 1024), (1024, 1), 0), reinterpret_tensor(buf7, (1, 256, 1024), (262144, 1024, 1), 0), None, )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_3 = rand_strided((1, 256), (256, 1), device='cuda:0', dtype=torch.int64)
    view = rand_strided((256, 1024), (1024, 1), device='cuda:0', dtype=torch.float32)
    sub_1 = rand_strided((256, 50265), (50265, 1), device='cuda:0', dtype=torch.float32)
    ...
    tangents_2 = rand_strided((1, 256, 50265), (12867840, 50265, 1), device='cuda:0', dtype=torch.float32)
