...
def forward(self, primals_1: "f32[50265, 1024]", primals_2: "f32[1, 256, 1024]", primals_3: "i64[1, 256]"):
    ...
    view: "f32[256, 1024]" = torch.ops.aten.reshape.default(primals_2, [256, 1024]);  primals_2 = None
    mm: "f32[256, 50265]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 256, 50265]" = torch.ops.aten.reshape.default(mm, [1, 256, 50265]);  mm = None
    ...
    view_2: "f32[256, 50265]" = torch.ops.aten.reshape.default(view_1, [-1, 50265])
    view_3: "i64[256]" = torch.ops.aten.reshape.default(primals_3, [-1])
    ...
    where: "i64[256]" = torch.ops.aten.where.self(ne, view_3, full_default);  view_3 = full_default = None
    unsqueeze: "i64[256, 1]" = torch.ops.aten.unsqueeze.default(where, 1);  where = None
    gather: "f32[256, 1]" = torch.ops.aten.gather.default(sub_1, 1, unsqueeze);  unsqueeze = None
