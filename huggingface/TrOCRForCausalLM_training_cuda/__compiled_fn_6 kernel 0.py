...
# kernel path: /tmp/torchinductor_youkaichao/lk/clk77dncdp27vgqyidtlfyuiozpoejy3oux7kyr5kudqlabxh7qi.py
...
# kernel path: /tmp/torchinductor_youkaichao/cb/ccbaljr5av6xns6cbb5sbqiyq5i4er6zcgxv3apd5j2wqphfse6z.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf0 = empty((1, 256), device='cuda', dtype=torch.int64)
        # Source Nodes: [add], Original ATen: [aten.add]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_add_0.run(buf0, 256, grid=grid(256), stream=stream0)
        buf1 = empty((1, 256, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [embedding], Original ATen: [aten.embedding]
        triton_poi_fused_embedding_1.run(primals_1, buf1, 262144, grid=grid(262144), stream=stream0)
        del primals_1
        return (buf1, buf0, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_1 = rand_strided((514, 1024), (1024, 1), device='cuda:0', dtype=torch.float32)
