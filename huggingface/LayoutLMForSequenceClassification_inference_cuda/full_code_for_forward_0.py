...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140449776628928)) \
        ...
        and (___check_type_id(L['outputs'], 143699248)) \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140449776628928)) \
        and (L['self'].training == False) \
        and (___check_type_id(L['labels'], 87704992)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 87704992)) \
        ...
        and (___check_type_id(G['torch'].long, 140451768542976)) \
