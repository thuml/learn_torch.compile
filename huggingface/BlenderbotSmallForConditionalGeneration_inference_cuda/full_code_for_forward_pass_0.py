...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140428628936112)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 146096784)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'decoder_input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 14805072)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140425350225424))) \
        and (___compile_config_hash() == '5df9378f78314ccf39023efca5d5b667') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.long, 140430611691264)) \
        and (G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.float16, 140430611691264)) \
        and (G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small']._make_causal_mask.__defaults__[0], 7640416)) \
        and (G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small']._make_causal_mask.__defaults__[0] == 0) \
