...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140649877486176)) \
        ...
        and (___check_type_id(L['transformer_outputs'], 129496800)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140646612704784))) \
        and (___compile_config_hash() == 'b095e4e5edde2afc9f67840e42c6ef67') \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140649877486176)) \
        and (L['self'].training == False) \
        and (___check_type_id(L['labels'], 73508256)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 73508256)) \
        ...
        and (___check_type_id(G['torch'].long, 140651874432768)) \
        and (G['torch'].long == torch.int64) \
        and (___check_type_id(G['torch'].float32, 140651874432768)) \
