...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139934077237808)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 155541664)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 24257872)) \
        and (___check_type_id(L['self'].grad_scaler, 141282848)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 86118640)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139930806689296))) \
        and (___compile_config_hash() == '58123ca784c3c437ba638c68602859dc') \
