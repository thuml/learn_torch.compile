...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139952243916416)) \
        ...
        and (___check_type_id(L['transformer_outputs'], 140717296)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139948973956624))) \
        and (___compile_config_hash() == 'f4f277a6bcce744e1f57a271821e6d06') \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139952243916416)) \
        and (L['self'].training == False) \
        and (___check_type_id(L['labels'], 84710768)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 84710768)) \
        ...
        and (___check_type_id(G['torch'].long, 139954235533056)) \
