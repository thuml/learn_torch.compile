...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139825894542880)) \
        ...
        and (___check_type_id(L['outputs'], 141065968)) \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139825894542880)) \
        and (L['self'].training == True) \
        and (___check_type_id(L['labels'], 85122368)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 85122368)) \
        ...
        and (___check_type_id(G['torch'].long, 139827885958912)) \
