...
def forward(self, primals_2: "i64[1]", sub_1: "f32[1, 2]", ne: "b8[1]", tangents_1: "f32[]"):
    ...
    view_1: "i64[1]" = torch.ops.aten.reshape.default(primals_2, [-1]);  primals_2 = None
    ...
    full_default_3: "f32[1, 2]" = torch.ops.aten.full.default([1, 2], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    exp_1: "f32[1, 2]" = torch.ops.aten.exp.default(sub_1);  sub_1 = None
