...
def forward(self, hidden_states, attention_mask, encoder_hidden_states, encoder_attention_mask, layer_head_mask, cross_attn_layer_head_mask, past_key_value, output_attentions, use_cache):
    ...
    __temp_1244 = self.self_attn(hidden_states=hidden_states, past_key_value=
    ...
    self_attn_weights = __temp_1244[1]
    present_key_value = __temp_1244[2]
    ...
    if encoder_hidden_states is not None:
        ...
        __temp_1247 = self.encoder_attn(hidden_states=hidden_states,
            ...
            output_attentions)
        hidden_states = __temp_1247[0]
        cross_attn_weights = __temp_1247[1]
        cross_attn_present_key_value = __temp_1247[2]
        hidden_states = nn.functional.dropout(hidden_states, p=self.dropout,
            training=self.training)
        hidden_states = residual + hidden_states
