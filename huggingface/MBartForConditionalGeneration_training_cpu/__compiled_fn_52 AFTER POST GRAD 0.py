...
def forward(self, primals_1: "f32[50265, 1024]", primals_2: "f32[1, 50265]", primals_3: "f32[1, 1024, 1024]", primals_4: "i64[1, 1024]"):
    ...
    view: "f32[1024, 1024]" = torch.ops.aten.reshape.default(primals_3, [1024, 1024]);  primals_3 = None
    mm: "f32[1024, 50265]" = torch.ops.aten.mm.default(view, permute)
    view_1: "f32[1, 1024, 50265]" = torch.ops.aten.reshape.default(mm, [1, 1024, 50265]);  mm = None
    ...
    view_2: "f32[1024, 50265]" = torch.ops.aten.reshape.default(add, [-1, 50265])
    view_3: "i64[1024]" = torch.ops.aten.reshape.default(primals_4, [-1])
    ...
    where: "i64[1024]" = torch.ops.aten.where.self(ne, view_3, full_default);  view_3 = full_default = None
    unsqueeze: "i64[1024, 1]" = torch.ops.aten.unsqueeze.default(where, 1);  where = None
    gather: "f32[1024, 1]" = torch.ops.aten.gather.default(sub_1, 1, unsqueeze);  unsqueeze = None
