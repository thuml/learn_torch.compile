...
def forward(self, primals_1: "f32[50265, 768]", primals_2: "f32[4098, 768]", primals_3: "f32[1, 768]", primals_4: "f32[768]", primals_5: "f32[768]", primals_6: "i64[1, 1024]"):
    ...
    full_default: "i64[1, 1024]" = torch.ops.aten.full.default([1, 1024], 0, dtype = torch.int64, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    ...
    full_default_1: "f32[1, 1024]" = torch.ops.aten.full.default([1, 1024], -0.0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
