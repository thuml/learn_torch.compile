...
def forward(self, primals, tangents):
    primals_1: "f32[1026, 1024]"; tangents_1: "f32[1, 1024, 1024]"; 
    ...
    where: "f32[1, 1024, 1024]" = torch.ops.aten.where.self(unsqueeze, scalar_tensor, tangents_1);  unsqueeze = scalar_tensor = tangents_1 = None
    full: "f32[1026, 1024]" = torch.ops.aten.full.default([1026, 1024], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    _unsafe_index_put: "f32[1026, 1024]" = torch.ops.aten._unsafe_index_put.default(full, [add], where, True);  full = add = where = None
