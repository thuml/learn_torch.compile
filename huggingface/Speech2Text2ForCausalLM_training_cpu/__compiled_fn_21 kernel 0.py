                       ...
                       long* out_ptr1)
{
    #pragma omp parallel num_threads(28)
    {
        {
            #pragma omp for 
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(1280000L); x0+=static_cast<long>(8L))
            {
    ...
    #pragma omp parallel num_threads(28)
    {
        ...
        }
        {
            ...
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(128L); x0+=static_cast<long>(1L))
            {
                for(long x1=static_cast<long>(0L); x1<static_cast<long>(10000L); x1+=static_cast<long>(8L))
                {
                    auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr4 + static_cast<long>(x1 + (10000L*x0)));
                    auto tmp1 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<long>(x1 + (10000L*x0)));
                    auto tmp2 = in_ptr1[static_cast<long>(x0)];
                    auto tmp5 = in_ptr2[static_cast<long>(0L)];
                    auto tmp6 = in_ptr3[static_cast<long>(0L)];
                    auto tmp12 = at::vec::Vectorized<float>::loadu(in_ptr5 + static_cast<long>(x1 + (10000L*x0)));
                    ...
                    tmp18.store(in_out_ptr0 + static_cast<long>(x1 + (10000L*x0)));
                }
            }
        }
    }
...
def call(args):
    ...
    buf5 = reinterpret_tensor(buf3, (1, 128, 10000), (1280000, 10000, 1), 0); del buf3  # reuse
    ...
    del tangents_1
    del tangents_2
    buf6 = empty((10000, 256), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (10000, 128), (1, 10000), 0), view, out=buf6)
    del view
    buf7 = empty((128, 256), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (128, 10000), (10000, 1), 0), permute_3, out=buf7)
    del buf5
    del permute_3
    return (reinterpret_tensor(buf6, (10000, 256), (256, 1), 0), reinterpret_tensor(buf7, (1, 128, 256), (32768, 256, 1), 0), None, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    sub_1 = rand_strided((128, 10000), (10000, 1), device='cpu', dtype=torch.float32)
    convert_element_type = rand_strided((), (), device='cpu', dtype=torch.float32)
    permute_3 = rand_strided((10000, 256), (256, 1), device='cpu', dtype=torch.float32)
    tangents_1 = rand_strided((), (), device='cpu', dtype=torch.float32)
    tangents_2 = rand_strided((1, 128, 10000), (1280000, 10000, 1), device='cpu', dtype=torch.float32)
