...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 158084448)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140116092468336)) \
        ...
        and (___check_obj_id(L['self'].autocast, 26803200)) \
        and (___check_type_id(L['self'].grad_scaler, 143801232)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 88664112)) \
