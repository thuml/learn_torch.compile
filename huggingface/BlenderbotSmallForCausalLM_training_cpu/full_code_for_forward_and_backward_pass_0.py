...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 155088912)) \
        and (___check_obj_id(L['___stack0'], 23797248)) \
        and (___check_type_id(L['___stack1'], 141697456)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 140822416)) \
...
def __resume_at_20_1(___stack0, self, mod, collect_outputs, cloned_inputs):
    with self.autocast() as __temp_61:
        __temp_63 = {}
        __temp_63.update(cloned_inputs)
        pred = mod(*(), **__temp_63)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_73:
        __temp_75 = {}
        __temp_75.update(cloned_inputs)
        pred = mod(*(), **__temp_75)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_86:
        __temp_88 = {}
        __temp_88.update(cloned_inputs)
        pred = mod(*(), **__temp_88)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
