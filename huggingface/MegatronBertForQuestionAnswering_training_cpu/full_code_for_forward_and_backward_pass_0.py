...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140261486211664)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 168340288)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'start_positions', 'end_positions', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 37051824)) \
        and (___check_type_id(L['self'].grad_scaler, 154051216)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 98911920)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140258222349840))) \
        and (___compile_config_hash() == 'b4c939214a550626d94c13969f85a2c6') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_megatron_bert_dot_modeling_megatron_bert'].torch.long, 140263483827968)) \
