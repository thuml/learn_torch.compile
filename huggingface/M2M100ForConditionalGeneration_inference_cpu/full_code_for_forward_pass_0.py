...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140038769261968)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 167521632)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'labels', 'input_ids', 'decoder_input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 36236816)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140035504135696))) \
        and (___compile_config_hash() == '52aee4a136e9ad5711aec26c226f099d') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_m2m_100_dot_modeling_m2m_100'].torch.float16, 140040765814528)) \
