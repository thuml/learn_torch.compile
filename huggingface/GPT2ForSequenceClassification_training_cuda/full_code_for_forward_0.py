...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140581211340224)) \
        ...
        and (___check_type_id(L['transformer_outputs'], 149990480)) \
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140581211340224)) \
        and (L['self'].training == True) \
        and (___check_type_id(L['labels'], 94000560)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 94000560)) \
        ...
        and (___check_type_id(G['torch'].long, 140583202830080)) \
