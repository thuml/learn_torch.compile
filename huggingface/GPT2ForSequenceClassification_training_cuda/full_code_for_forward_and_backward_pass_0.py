...
def __resume_at_108_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    'Failed to decompile.'

def transformed___resume_at_108_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_9(___stack0, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_108_9 ============#

def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 163425360)) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140577941315088))) \
        and (___compile_config_hash() == 'd0e202b882cf71cac6f5a26cca65461e') \
...
def __transformed_code_3_for_resume_in_forward_and_backward_pass(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_9(self.optimizer_step(), mod, collect_outputs,
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 163425360)) \
        and (___check_obj_id(L['___stack0'], 32140288)) \
        and (___check_type_id(L['___stack1'], 150029136)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 149130800)) \
        ...
        and (2 <= L['___stack1'].past_key_values[0][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[0][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[1][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[1][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[2][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[2][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[3][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[3][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[4][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[4][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[5][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[5][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[6][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[6][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[7][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[7][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[8][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[8][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[9][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[9][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[10][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[10][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[11][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[11][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[0][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[1][0].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[1][1].storage_offset()) \
        and (2 <= L['___stack1'].past_key_values[2][0].storage_offset()) \
...
def __resume_at_20_1(___stack0, self, mod, collect_outputs, cloned_inputs):
    with self.autocast() as __temp_35:
        __temp_37 = {}
        __temp_37.update(cloned_inputs)
        pred = mod(*(), **__temp_37)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_47:
        __temp_49 = {}
        __temp_49.update(cloned_inputs)
        pred = mod(*(), **__temp_49)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_60:
        __temp_62 = {}
        __temp_62.update(cloned_inputs)
        pred = mod(*(), **__temp_62)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
