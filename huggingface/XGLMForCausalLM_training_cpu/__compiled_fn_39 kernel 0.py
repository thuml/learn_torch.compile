                       ...
                       long* out_ptr1)
{
    #pragma omp parallel num_threads(28)
    {
        {
            #pragma omp for 
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(32769024L); x0+=static_cast<long>(8L))
            {
    ...
    #pragma omp parallel num_threads(28)
    {
        ...
        }
        {
            ...
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(128L); x0+=static_cast<long>(1L))
            {
                for(long x1=static_cast<long>(0L); x1<static_cast<long>(256008L); x1+=static_cast<long>(8L))
                {
                    auto tmp0 = at::vec::Vectorized<float>::loadu(in_ptr4 + static_cast<long>(x1 + (256008L*x0)));
                    auto tmp1 = at::vec::Vectorized<float>::loadu(in_ptr0 + static_cast<long>(x1 + (256008L*x0)));
                    auto tmp2 = in_ptr1[static_cast<long>(x0)];
                    auto tmp5 = in_ptr2[static_cast<long>(0L)];
                    auto tmp6 = in_ptr3[static_cast<long>(0L)];
                    auto tmp12 = at::vec::Vectorized<float>::loadu(in_ptr5 + static_cast<long>(x1 + (256008L*x0)));
                    ...
                    tmp18.store(in_out_ptr0 + static_cast<long>(x1 + (256008L*x0)));
                }
            }
        }
    }
...
def call(args):
    ...
    cpp_fused_nll_loss_backward_nll_loss_forward_0(c_void_p(slice_scatter_2.data_ptr()), c_void_p(buf0.data_ptr()), c_void_p(buf1.data_ptr()))
    aten.scatter_(buf0,1,buf1,-1.0)
    del buf1
    buf4 = empty_strided((128, 1), (1, 128), device='cpu', dtype=torch.float32)
    buf3 = empty((128, 256008), device='cpu', dtype=torch.float32)
    buf5 = reinterpret_tensor(buf3, (1, 128, 256008), (32769024, 256008, 1), 0); del buf3  # reuse
    cpp_fused__log_softmax_backward_data_add_nll_loss_backward_nll_loss_forward_1(c_void_p(buf5.data_ptr()), c_void_p(buf0.data_ptr()), c_void_p(slice_scatter_2.data_ptr()), c_void_p(tangents_1.data_ptr()), c_void_p(convert_element_type.data_ptr()), c_void_p(tangents_2.data_ptr()), c_void_p(sub_1.data_ptr()), c_void_p(buf4.data_ptr()))
    del buf0
    del buf4
    ...
    del tangents_1
    del tangents_2
    buf6 = empty((256008, 1024), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (256008, 128), (1, 256008), 0), view, out=buf6)
    del view
    buf7 = empty((128, 1024), device='cpu', dtype=torch.float32)
    # Source Nodes: [], Original ATen: [aten.mm]
    extern_kernels.mm(reinterpret_tensor(buf5, (128, 256008), (256008, 1), 0), permute_3, out=buf7)
    del buf5
    del permute_3
    return (reinterpret_tensor(buf6, (256008, 1024), (1024, 1), 0), reinterpret_tensor(buf7, (1, 128, 1024), (131072, 1024, 1), 0), None, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    slice_scatter_2 = rand_strided((1, 128), (128, 1), device='cpu', dtype=torch.int64)
    sub_1 = rand_strided((128, 256008), (256008, 1), device='cpu', dtype=torch.float32)
    convert_element_type = rand_strided((), (), device='cpu', dtype=torch.float32)
    permute_3 = rand_strided((256008, 1024), (1024, 1), device='cpu', dtype=torch.float32)
    tangents_1 = rand_strided((), (), device='cpu', dtype=torch.float32)
    tangents_2 = rand_strided((1, 128, 256008), (32769024, 256008, 1), device='cpu', dtype=torch.float32)
