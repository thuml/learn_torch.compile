...
def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139780594496752)) \
        ...
        and (___check_type_id(L['outputs'], 141577680)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139777329831440))) \
        and (___compile_config_hash() == 'c3264ae975a2956c14ce4f7e76400e0c') \
...
def __resume_at_164_3(self, labels, return_dict, outputs, logits, loss):
...
def transformed___resume_at_164_3(self, labels, return_dict, outputs, logits, loss):
...
def __guard_0_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 139780594496752)) \
        and (L['self'].training == False) \
        and (___check_type_id(L['labels'], 85789088)) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 85789088)) \
        ...
        and (___check_type_id(G['torch'].long, 139782591825664)) \
