...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 159867056)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139743488312176)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 159867056)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 28579520)) \
        and (___check_type_id(L['self'].grad_scaler, 145592832)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.long, 139745481209600)) \
        and (G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.float, 139745481209600)) \
