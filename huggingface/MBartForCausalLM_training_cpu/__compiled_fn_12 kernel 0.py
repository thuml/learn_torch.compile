...
cpp_fused_add_native_layer_norm_native_layer_norm_backward_7 = async_compile.cpp('''
#include "/tmp/torchinductor_youkaichao/2l/c2ljzlm4sosod7u6lyrroqdba6hmfcyijrric6p4t3fhbcmw6osp.h"
extern "C" void kernel(float* in_out_ptr0,
                       float* in_out_ptr1,
                       const float* in_ptr0,
...
def call(args):
    ...
    buf11 = empty((16, 1024, 1024), device='cpu', dtype=torch.float32)
    # Source Nodes: [attn_weights], Original ATen: [aten.bmm]
    extern_kernels.bmm(reinterpret_tensor(buf10, (16, 1024, 64), (65536, 64, 1), 0), reinterpret_tensor(buf7, (16, 64, 1024), (65536, 1, 64), 0), out=buf11)
    buf12 = empty_strided((16, 1024, 1), (1024, 1, 16384), device='cpu', dtype=torch.float32)
    buf13 = buf11; del buf11  # reuse
    buf14 = empty_strided((16, 1024, 1), (1024, 1, 16384), device='cpu', dtype=torch.float32)
    buf15 = empty((16, 1024, 1024), device='cpu', dtype=torch.float32)
    buf35 = empty((16, 1024, 1024), device='cpu', dtype=torch.float32)
    cpp_fused__softmax_clone_detach_3(c_void_p(buf13.data_ptr()), c_void_p(primals_18.data_ptr()), c_void_p(buf12.data_ptr()), c_void_p(buf14.data_ptr()), c_void_p(buf15.data_ptr()), c_void_p(buf35.data_ptr()))
    del buf12
    del buf13
    del buf14
    del primals_18
    buf16 = reinterpret_tensor(buf8, (16, 1024, 64), (65536, 64, 1), 0); del buf8  # reuse
    # Source Nodes: [attn_output], Original ATen: [aten.bmm]
    extern_kernels.bmm(buf15, reinterpret_tensor(buf9, (16, 1024, 64), (65536, 64, 1), 0), out=buf16)
    ...
    cpp_fused_add_native_layer_norm_view_5(c_void_p(primals_17.data_ptr()), c_void_p(buf20.data_ptr()), c_void_p(primals_11.data_ptr()), c_void_p(primals_12.data_ptr()), c_void_p(buf22.data_ptr()), c_void_p(buf23.data_ptr()), c_void_p(buf25.data_ptr()), c_void_p(buf26.data_ptr()))
    del buf22
    del primals_12
    buf27 = empty((1024, 4096), device='cpu', dtype=torch.float32)
    # Source Nodes: [l__self___fc1], Original ATen: [aten.addmm]
    extern_kernels.addmm(primals_14, buf26, reinterpret_tensor(primals_13, (1024, 4096), (1, 1024), 0), alpha=1, beta=1, out=buf27)
    del primals_14
    ...
    extern_kernels.addmm(primals_16, buf28, reinterpret_tensor(primals_15, (4096, 1024), (1, 4096), 0), alpha=1, beta=1, out=buf29)
    del primals_16
    # Source Nodes: [hidden_states_8], Original ATen: [aten.native_dropout]
    buf30 = aten.native_dropout(reinterpret_tensor(buf29, (1, 1024, 1024), (1048576, 1024, 1), 0), 0.1, True)
    del buf29
    buf31 = buf30[0]
    buf32 = buf30[1]
    del buf30
    buf33 = buf31; del buf31  # reuse
    buf34 = reinterpret_tensor(buf23, (1, 1024, 1), (1024, 1, 1), 0); del buf23  # reuse
    cpp_fused_add_native_layer_norm_native_layer_norm_backward_7(c_void_p(buf33.data_ptr()), c_void_p(buf34.data_ptr()), c_void_p(primals_17.data_ptr()), c_void_p(buf20.data_ptr()))
    return (buf33, buf7, buf9, primals_1, primals_11, primals_17, buf0, buf3, buf4, buf17, buf21, buf25, buf26, buf27, buf28, buf32, reinterpret_tensor(primals_15, (1024, 4096), (4096, 1), 0), reinterpret_tensor(primals_13, (4096, 1024), (1024, 1), 0), buf34, reinterpret_tensor(primals_9, (1024, 1024), (1024, 1), 0), reinterpret_tensor(buf15, (16, 1024, 1024), (1048576, 1, 1024), 0), reinterpret_tensor(buf9, (16, 64, 1024), (65536, 1, 64), 0), buf35, reinterpret_tensor(buf10, (16, 64, 1024), (65536, 1, 64), 0), reinterpret_tensor(buf7, (16, 1024, 64), (65536, 64, 1), 0), reinterpret_tensor(primals_7, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_5, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_3, (1024, 1024), (1024, 1), 0), )
