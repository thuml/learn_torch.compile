...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139912183364336)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 172249344)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 40951104)) \
        and (___check_type_id(L['self'].grad_scaler, 157994080)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139908909784592))) \
        and (___compile_config_hash() == 'b635d71fbbce2bb6daa5cfde9450f3f7') \
        ...
        and (G['__import_transformers_dot_modeling_utils'].ENV_VARS_TRUE_VALUES == {'TRUE', 'ON', '1', 'YES'}) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.long, 139914171422464)) \
        and (G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_xlnet_dot_modeling_xlnet'].torch.float, 139914171422464)) \
...
def __transformed_code_1_for_resume_in_forward_and_backward_pass(___stack0, self, mod, collect_outputs, cloned_inputs):
    ...
    import importlib
    loss = graph_out_0[0]
    pred = importlib.import_module('transformers.models.xlnet.modeling_xlnet'
        ).XLNetLMHeadModelOutput(loss=graph_out_0[0], logits=graph_out_0[1],
        mems=None, hidden_states=None, attentions=None)
