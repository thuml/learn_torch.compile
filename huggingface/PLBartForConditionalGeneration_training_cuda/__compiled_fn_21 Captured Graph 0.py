...
def forward(self, L_hidden_states_ : torch.Tensor, L_attention_mask_ : torch.Tensor, L_encoder_hidden_states_ : torch.Tensor):
    ...
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/plbart/modeling_plbart.py:168, code: return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
    view_7 = l__self___encoder_attn_k_proj.view(1, -1, 12, 64);  l__self___encoder_attn_k_proj = None
    ...
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/plbart/modeling_plbart.py:168, code: return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
    view_8 = l__self___encoder_attn_v_proj.view(1, -1, 12, 64);  l__self___encoder_attn_v_proj = None
    transpose_6 = view_8.transpose(1, 2);  view_8 = None
    value_states_2 = transpose_6.contiguous();  transpose_6 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/plbart/modeling_plbart.py:168, code: return tensor.view(bsz, seq_len, self.num_heads, self.head_dim).transpose(1, 2).contiguous()
    view_9 = query_states_2.view(1, 1024, 12, 64);  query_states_2 = None
    ...
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/plbart/modeling_plbart.py:269, code: attn_probs = nn.functional.dropout(attn_weights, p=self.dropout, training=self.training)
    attn_probs_1 = torch.nn.functional.dropout(attn_weights_5, p = 0.1, training = True);  attn_weights_5 = None
    ...
    return (hidden_states_13, key_states, value_states, key_states_2, value_states_2)
