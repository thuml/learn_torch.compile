...
# kernel path: /tmp/torchinductor_youkaichao/iq/ciqewfa6ve6jevnaks55z2zmfry4j5qvbh7jf64tkkufh4enwieu.py
...
# kernel path: /tmp/torchinductor_youkaichao/wt/cwtwbmp3qwqczmwqwfu5glfkc2evhc63kwcpgiln2zntgyeoyw5t.py
# Source Nodes: [], Original ATen: []
...
# kernel path: /tmp/torchinductor_youkaichao/sa/csa4uky6djt6v3kxoizfutxwj7a3tpy3kuo4hnff2z6bw3fnj75w.py
...
# kernel path: /tmp/torchinductor_youkaichao/yw/cywhtu7yvtf3nh44nprbp5xw3nmmqrhah7bu7ycx5m47s64t7pid.py
...
# kernel path: /tmp/torchinductor_youkaichao/k5/ck5dp7ix4udylleem7s7ssxw2cphuyu3s2lwc7ofd3eky4puce7x.py
...
# kernel path: /tmp/torchinductor_youkaichao/2y/c2y5asbiqkntrjunsn3jojofrjlyblp4dgcvkr5gzh2pgywrntce.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_17, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_5, (768, 768), (1, 768), 0), out=buf2)
        buf3 = empty((1, 12, 1024, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        stream0 = get_cuda_stream(0)
        triton_poi_fused_0.run(buf0, primals_2, buf3, 786432, grid=grid(786432), stream=stream0)
        del primals_2
        buf4 = reinterpret_tensor(buf0, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0); del buf0  # reuse
        # Source Nodes: [], Original ATen: []
        triton_poi_fused_1.run(buf1, primals_4, buf4, 786432, grid=grid(786432), stream=stream0)
        del primals_4
        buf5 = reinterpret_tensor(buf1, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0); del buf1  # reuse
        # Source Nodes: [], Original ATen: []
        triton_poi_fused_1.run(buf2, primals_6, buf5, 786432, grid=grid(786432), stream=stream0)
        del primals_6
        # Source Nodes: [], Original ATen: []
        buf6 = aten._scaled_dot_product_efficient_attention(buf3, buf4, buf5, None, True, 0.1, scale=1.0)
        buf7 = buf6[0]
        buf8 = buf6[1]
        buf9 = buf6[2]
        buf10 = buf6[3]
        ...
        # Source Nodes: [hidden_states], Original ATen: [aten.view]
        triton_poi_fused_view_2.run(buf7, buf11, 786432, grid=grid(786432), stream=stream0)
        buf12 = empty((1024, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_8, buf11, reinterpret_tensor(primals_7, (768, 768), (1, 768), 0), alpha=1, beta=1, out=buf12)
        del primals_8
        # Source Nodes: [hidden_states_1], Original ATen: [aten.native_dropout]
        buf13 = aten.native_dropout(reinterpret_tensor(buf12, (1, 1024, 768), (786432, 768, 1), 0), 0.1, True)
        buf14 = buf13[0]
        buf15 = buf13[1]
        del buf13
        buf19 = reinterpret_tensor(buf12, (1, 1024, 768), (786432, 768, 1), 0); del buf12  # reuse
        buf20 = empty((1024, 768), device='cuda', dtype=torch.float32)
        buf33 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_2, l__self___fc1, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_3.run(primals_17, buf14, primals_9, primals_10, buf19, buf20, buf33, 1024, 768, grid=grid(1024), stream=stream0)
        buf21 = empty((1024, 3072), device='cuda', dtype=torch.float32)
        # Source Nodes: [l__self___fc1], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_12, buf20, reinterpret_tensor(primals_11, (768, 3072), (1, 768), 0), alpha=1, beta=1, out=buf21)
        del primals_12
        buf22 = empty((1024, 3072), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_4, hidden_states_6], Original ATen: [aten.gelu, aten.view]
        triton_poi_fused_gelu_view_4.run(buf21, buf22, 3145728, grid=grid(3145728), stream=stream0)
        buf23 = reinterpret_tensor(buf14, (1024, 768), (768, 1), 0); del buf14  # reuse
        # Source Nodes: [hidden_states_6], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_14, buf22, reinterpret_tensor(primals_13, (3072, 768), (1, 3072), 0), alpha=1, beta=1, out=buf23)
        del primals_14
        # Source Nodes: [hidden_states_7], Original ATen: [aten.native_dropout]
        buf24 = aten.native_dropout(reinterpret_tensor(buf23, (1, 1024, 768), (786432, 768, 1), 0), 0.1, True)
        buf25 = buf24[0]
        buf26 = buf24[1]
        del buf24
        buf30 = reinterpret_tensor(buf23, (1, 1024, 768), (786432, 768, 1), 0); del buf23  # reuse
        buf31 = empty((1, 1024, 768), device='cuda', dtype=torch.float32)
        buf32 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_8, hidden_states_9, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_5.run(buf19, primals_9, primals_10, buf25, primals_15, primals_16, buf30, buf31, buf32, 1024, 768, grid=grid(1024), stream=stream0)
        del buf25
        del primals_10
        del primals_16
        return (buf31, primals_9, primals_15, reinterpret_tensor(primals_17, (1024, 768), (768, 1), 0), buf3, buf4, buf5, buf8, buf9, buf10, buf7, buf11, buf15, buf19, buf20, buf21, buf22, buf26, buf30, buf32, reinterpret_tensor(primals_13, (768, 3072), (3072, 1), 0), reinterpret_tensor(primals_11, (3072, 768), (768, 1), 0), buf33, reinterpret_tensor(primals_7, (768, 768), (768, 1), 0), reinterpret_tensor(primals_5, (768, 768), (768, 1), 0), reinterpret_tensor(primals_3, (768, 768), (768, 1), 0), reinterpret_tensor(primals_1, (768, 768), (768, 1), 0), )
