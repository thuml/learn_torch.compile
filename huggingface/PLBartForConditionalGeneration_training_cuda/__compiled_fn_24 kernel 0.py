...
# kernel path: /tmp/torchinductor_youkaichao/vc/cvc4dnl7y4gajvpce4lj4tmpx5gztpe6mczyfhswtblcngqx6zui.py
...
# kernel path: /tmp/torchinductor_youkaichao/b7/cb76uf6u4cq5r2h7rw6yid4ozsar6tegaveey7b23p5rtr4svje2.py
...
# kernel path: /tmp/torchinductor_youkaichao/xz/cxzaed5m5fw2ze4xwl35xgm2yhvv5byhyjqte6j2zbwutklbx4e3.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        extern_kernels.mm(reinterpret_tensor(primals_27, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_1, (768, 768), (1, 768), 0), out=buf0)
        buf1 = empty((1024, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_27, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_3, (768, 768), (1, 768), 0), out=buf1)
        ...
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_27, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_5, (768, 768), (1, 768), 0), out=buf3)
        ...
        # Source Nodes: [attn_weights_3], Original ATen: [aten._softmax]
        triton_per_fused__softmax_2.run(buf6, primals_28, buf9, 12288, 1024, grid=grid(12288), stream=stream0)
        del buf6
        del primals_28
        ...
        buf58 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_2, l__self___encoder_attn_q_proj, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_4.run(primals_27, buf17, primals_9, primals_10, buf22, buf23, buf58, 1024, 768, grid=grid(1024), stream=stream0)
        buf24 = reinterpret_tensor(buf17, (1024, 768), (768, 1), 0); del buf17  # reuse
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(buf23, reinterpret_tensor(primals_11, (768, 768), (1, 768), 0), out=buf24)
        buf25 = empty((1024, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_29, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_13, (768, 768), (1, 768), 0), out=buf25)
        buf26 = empty((1, 12, 1024, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [key_states_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_0.run(buf25, primals_14, buf26, 786432, grid=grid(786432), stream=stream0)
        del primals_14
        buf27 = buf25; del buf25  # reuse
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_29, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_15, (768, 768), (1, 768), 0), out=buf27)
        buf28 = empty((1, 12, 1024, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [value_states_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_0.run(buf27, primals_16, buf28, 786432, grid=grid(786432), stream=stream0)
        del primals_16
        buf29 = reinterpret_tensor(buf27, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0); del buf27  # reuse
        # Source Nodes: [], Original ATen: []
        triton_poi_fused_clone_1.run(buf24, primals_12, buf29, 786432, grid=grid(786432), stream=stream0)
        del primals_12
        # Source Nodes: [], Original ATen: []
        buf30 = aten._scaled_dot_product_efficient_attention(buf29, reinterpret_tensor(buf26, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0), reinterpret_tensor(buf28, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0), None, True, 0.1, scale=1.0)
        buf31 = buf30[0]
        buf32 = buf30[1]
        buf33 = buf30[2]
        buf34 = buf30[3]
        ...
        # Source Nodes: [hidden_states_4], Original ATen: [aten.view]
        triton_poi_fused_view_5.run(buf31, buf35, 786432, grid=grid(786432), stream=stream0)
        buf36 = empty((1024, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_4], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_18, buf35, reinterpret_tensor(primals_17, (768, 768), (1, 768), 0), alpha=1, beta=1, out=buf36)
        del primals_18
        # Source Nodes: [hidden_states_5], Original ATen: [aten.native_dropout]
        buf37 = aten.native_dropout(reinterpret_tensor(buf36, (1, 1024, 768), (786432, 768, 1), 0), 0.1, True)
        buf38 = buf37[0]
        buf39 = buf37[1]
        del buf37
        buf43 = reinterpret_tensor(buf36, (1, 1024, 768), (786432, 768, 1), 0); del buf36  # reuse
        buf44 = empty((1024, 768), device='cuda', dtype=torch.float32)
        buf57 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_6, l__self___fc1, residual_1, residual_2], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_6.run(buf22, primals_9, primals_10, buf38, primals_19, primals_20, buf43, buf44, buf57, 1024, 768, grid=grid(1024), stream=stream0)
        del primals_10
        buf45 = empty((1024, 3072), device='cuda', dtype=torch.float32)
        # Source Nodes: [l__self___fc1], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_22, buf44, reinterpret_tensor(primals_21, (768, 3072), (1, 768), 0), alpha=1, beta=1, out=buf45)
        del primals_22
        buf46 = empty((1024, 3072), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_10, hidden_states_8], Original ATen: [aten.gelu, aten.view]
        triton_poi_fused_gelu_view_7.run(buf45, buf46, 3145728, grid=grid(3145728), stream=stream0)
        buf47 = reinterpret_tensor(buf38, (1024, 768), (768, 1), 0); del buf38  # reuse
        # Source Nodes: [hidden_states_10], Original ATen: [aten.addmm]
        extern_kernels.addmm(primals_24, buf46, reinterpret_tensor(primals_23, (3072, 768), (1, 3072), 0), alpha=1, beta=1, out=buf47)
        del primals_24
        # Source Nodes: [hidden_states_11], Original ATen: [aten.native_dropout]
        buf48 = aten.native_dropout(reinterpret_tensor(buf47, (1, 1024, 768), (786432, 768, 1), 0), 0.1, True)
        buf49 = buf48[0]
        buf50 = buf48[1]
        del buf48
        buf54 = reinterpret_tensor(buf47, (1, 1024, 768), (786432, 768, 1), 0); del buf47  # reuse
        buf55 = empty((1, 1024, 768), device='cuda', dtype=torch.float32)
        buf56 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        ...
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_6.run(buf43, primals_19, primals_20, buf49, primals_25, primals_26, buf54, buf55, buf56, 1024, 768, grid=grid(1024), stream=stream0)
        del buf49
        del primals_20
        del primals_26
        return (buf55, buf2, buf4, buf26, buf28, primals_9, primals_19, primals_25, reinterpret_tensor(primals_27, (1024, 768), (768, 1), 0), buf12, buf14, buf18, buf22, buf23, reinterpret_tensor(primals_29, (1024, 768), (768, 1), 0), buf29, reinterpret_tensor(buf26, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0), reinterpret_tensor(buf28, (1, 12, 1024, 64), (786432, 65536, 64, 1), 0), buf32, buf33, buf34, buf31, buf35, buf39, buf43, buf44, buf45, buf46, buf50, buf54, buf56, reinterpret_tensor(primals_23, (768, 3072), (3072, 1), 0), reinterpret_tensor(primals_21, (3072, 768), (768, 1), 0), buf57, reinterpret_tensor(primals_17, (768, 768), (768, 1), 0), reinterpret_tensor(primals_15, (768, 768), (768, 1), 0), reinterpret_tensor(primals_13, (768, 768), (768, 1), 0), reinterpret_tensor(primals_11, (768, 768), (768, 1), 0), buf58, reinterpret_tensor(primals_7, (768, 768), (768, 1), 0), reinterpret_tensor(buf11, (12, 1024, 1024), (1048576, 1, 1024), 0), reinterpret_tensor(buf4, (12, 64, 1024), (65536, 1, 64), 0), buf9, reinterpret_tensor(buf5, (12, 64, 1024), (65536, 1, 64), 0), reinterpret_tensor(buf2, (12, 1024, 64), (65536, 64, 1), 0), reinterpret_tensor(primals_5, (768, 768), (768, 1), 0), reinterpret_tensor(primals_3, (768, 768), (768, 1), 0), reinterpret_tensor(primals_1, (768, 768), (768, 1), 0), )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_15 = rand_strided((768, 768), (768, 1), device='cuda:0', dtype=torch.float32)
    ...
    primals_21 = rand_strided((3072, 768), (768, 1), device='cuda:0', dtype=torch.float32)
    primals_22 = rand_strided((3072, ), (1, ), device='cuda:0', dtype=torch.float32)
    primals_23 = rand_strided((768, 3072), (3072, 1), device='cuda:0', dtype=torch.float32)
    ...
    primals_27 = rand_strided((1, 1024, 768), (786432, 768, 1), device='cuda:0', dtype=torch.float32)
    primals_28 = rand_strided((1, 1, 1024, 1024), (1048576, 1048576, 1024, 1), device='cuda:0', dtype=torch.float32)
    primals_29 = rand_strided((1, 1024, 768), (786432, 768, 1), device='cuda:0', dtype=torch.float32)
