...
def call(args):
    ...
    assert_size_stride(view_14, (128, 1024), (1024, 1))
    ...
    assert_size_stride(view_18, (128, 4096), (4096, 1))
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf0, (1024, 128), (1, 1024), 0), view_18, out=buf2)
        del view_18
        ...
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf4, (4096, 128), (1, 4096), 0), view_16, out=buf6)
        del view_16
        ...
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf13, (1024, 128), (1, 1024), 0), view_14, out=buf15)
        del view_14
        ...
        # Source Nodes: [attn_weights_1], Original ATen: [aten._softmax, aten._softmax_backward_data]
        triton_per_fused__softmax__softmax_backward_data_6.run(buf18, bmm, amax, sum_1, buf20, 2048, 128, grid=grid(2048), stream=stream0)
        del amax
        del bmm
        del buf18
        del sum_1
        ...
        # Source Nodes: [], Original ATen: [aten.bmm]
        extern_kernels.bmm(permute_26, buf20, out=buf21)
        del permute_26
        buf22 = empty((16, 128, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.bmm]
        extern_kernels.bmm(buf20, permute_27, out=buf22)
        del buf20
        del permute_27
        ...
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(buf23, permute_31, out=buf24)
        del permute_31
        ...
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf21, (128, 1024), (1, 128), 0), permute_36, out=buf27)
        del permute_36
        buf28 = empty((1024, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf21, (1024, 128), (128, 1), 0), view, out=buf28)
        ...
        buf30 = reinterpret_tensor(buf21, (128, 1024), (1024, 1), 0); del buf21  # reuse
        # Source Nodes: [], Original ATen: [aten.mul, aten.view]
        triton_poi_fused_mul_view_9.run(buf22, buf30, 131072, grid=grid(131072), stream=stream0)
        buf31 = reinterpret_tensor(buf22, (128, 1024), (1024, 1), 0); del buf22  # reuse
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(buf30, permute_40, out=buf31)
        del permute_40
        buf32 = empty((1024, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf30, (1024, 128), (1, 1024), 0), view, out=buf32)
        del view
        buf33 = empty((1, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.sum]
        triton_red_fused_sum_1.run(buf30, buf33, 1024, 128, grid=grid(1024), stream=stream0)
        del buf30
        buf39 = buf12; del buf12  # reuse
        # Source Nodes: [hidden_states], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_10.run(buf39, buf24, buf27, buf31, primals_1, primals_17, getitem_1, rsqrt, 128, 1024, grid=grid(128), stream=stream0)
        del primals_1
        buf37 = empty((1024, ), device='cuda', dtype=torch.float32)
        buf38 = empty((1024, ), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_11.run(buf24, buf27, buf31, primals_17, getitem_1, rsqrt, buf37, buf38, 1024, 128, grid=grid(1024), stream=stream0)
        del buf24
        del buf27
        del buf31
        del getitem_1
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    view_14 = rand_strided((128, 1024), (1024, 1), device='cuda:0', dtype=torch.float32)
    ...
    view_18 = rand_strided((128, 4096), (4096, 1), device='cuda:0', dtype=torch.float32)
