...
# kernel path: /tmp/torchinductor_youkaichao/hi/chixb7r6mhfedcjelgjpao7preitva75h4ouqx4ys7jv2tep4xvb.py
...
# kernel path: /tmp/torchinductor_youkaichao/r2/cr2jbronla5ji7thpawgoxz63mebl2jglyjtlxso5rarp6mh7xjq.py
...
# kernel path: /tmp/torchinductor_youkaichao/c3/cc3zznytoeqih3brmqo3vfswnqzmyoadtxpkma5jngfgar3u6hmj.py
...
# kernel path: /tmp/torchinductor_youkaichao/di/cdivdssgaus3m6zirpwudovmpe5dv7lso7zrkm2cgbkrjorysh73.py
...
# kernel path: /tmp/torchinductor_youkaichao/pi/cpixjhvtzpx2y2tzk5laefqelfhyemgwnmrcb5ruiuapljaaoymf.py
...
def call(args):
    ...
    assert_size_stride(primals_29, (1, 128, 1024), (131072, 1024, 1))
    with torch.cuda._DeviceGuard(0):
        ...
        triton_per_fused_native_layer_norm_view_0.run(buf3, primals_27, primals_1, primals_2, buf0, buf4, 128, 1024, grid=grid(128), stream=stream0)
        del primals_2
        ...
        # Source Nodes: [attn_probs, attn_weights_3], Original ATen: [aten._softmax, aten.clone, aten.detach]
        triton_per_fused__softmax_clone_detach_3.run(buf11, primals_28, buf14, buf56, 2048, 128, grid=grid(2048), stream=stream0)
        del primals_28
        ...
        # Source Nodes: [hidden_states_4, l__self___encoder_attn_q_proj, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_5.run(primals_27, buf19, primals_11, primals_12, buf24, buf25, buf55, 128, 1024, grid=grid(128), stream=stream0)
        del primals_12
        ...
        buf27 = empty((128, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_29, (128, 1024), (1024, 1), 0), reinterpret_tensor(primals_15, (1024, 1024), (1, 1024), 0), out=buf27)
        buf28 = empty((128, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_29, (128, 1024), (1024, 1), 0), reinterpret_tensor(primals_17, (1024, 1024), (1, 1024), 0), out=buf28)
        buf29 = empty((1, 16, 128, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [contiguous_5], Original ATen: [aten.clone]
        triton_poi_fused_clone_1.run(buf26, primals_14, buf29, 131072, grid=grid(131072), stream=stream0)
        del primals_14
        buf30 = reinterpret_tensor(buf26, (1, 16, 128, 64), (131072, 8192, 64, 1), 0); del buf26  # reuse
        # Source Nodes: [value_states_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_2.run(buf28, primals_18, buf30, 131072, grid=grid(131072), stream=stream0)
        del primals_18
        buf31 = reinterpret_tensor(buf28, (1, 16, 128, 64), (131072, 8192, 64, 1), 0); del buf28  # reuse
        # Source Nodes: [key_states_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_2.run(buf27, primals_16, buf31, 131072, grid=grid(131072), stream=stream0)
        del primals_16
        buf32 = buf11; del buf11  # reuse
        # Source Nodes: [attn_weights_4], Original ATen: [aten.bmm]
        extern_kernels.bmm(reinterpret_tensor(buf29, (16, 128, 64), (8192, 64, 1), 0), reinterpret_tensor(buf31, (16, 64, 128), (8192, 1, 64), 0), out=buf32)
        buf33 = empty((16, 128, 1), device='cuda', dtype=torch.float32)
        buf34 = empty((16, 128, 1), device='cuda', dtype=torch.float32)
        buf35 = empty((16, 128, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [attn_weights_5], Original ATen: [aten._softmax]
        triton_per_fused__softmax_6.run(buf32, buf33, buf34, buf35, 2048, 128, grid=grid(2048), stream=stream0)
        buf36 = reinterpret_tensor(buf27, (16, 128, 64), (8192, 64, 1), 0); del buf27  # reuse
        # Source Nodes: [attn_output_5], Original ATen: [aten.bmm]
        extern_kernels.bmm(buf35, reinterpret_tensor(buf30, (16, 128, 64), (8192, 64, 1), 0), out=buf36)
        buf37 = empty((128, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_5], Original ATen: [aten.view]
        triton_poi_fused_view_4.run(buf36, buf37, 131072, grid=grid(131072), stream=stream0)
        buf38 = reinterpret_tensor(buf36, (128, 1024), (1024, 1), 0); del buf36  # reuse
        ...
        buf39 = aten.native_dropout(reinterpret_tensor(buf38, (1, 128, 1024), (131072, 1024, 1), 0), 0.1, True)
        buf40 = buf39[0]
        buf41 = buf39[1]
        del buf39
        buf45 = reinterpret_tensor(buf38, (1, 128, 1024), (131072, 1024, 1), 0); del buf38  # reuse
        buf46 = empty((128, 1024), device='cuda', dtype=torch.float32)
        buf54 = empty((1, 128, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_8, l__self___fc1, residual_1, residual_2], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_7.run(primals_27, buf19, buf40, primals_21, primals_22, buf45, buf46, buf54, 128, 1024, grid=grid(128), stream=stream0)
        del primals_22
        buf47 = empty((128, 4096), device='cuda', dtype=torch.float32)
        ...
        del primals_24
        buf48 = empty((128, 4096), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_11, hidden_states_9], Original ATen: [aten.gelu, aten.view]
        triton_poi_fused_gelu_view_8.run(buf47, buf48, 524288, grid=grid(524288), stream=stream0)
        buf49 = empty((128, 1024), device='cuda', dtype=torch.float32)
        ...
        buf50 = aten.native_dropout(reinterpret_tensor(buf49, (1, 128, 1024), (131072, 1024, 1), 0), 0.1, True)
        del buf49
        buf51 = buf50[0]
        buf52 = buf50[1]
        del buf50
        ...
        # Source Nodes: [hidden_states_13, residual_1, residual_2], Original ATen: [aten.add]
        triton_poi_fused_add_9.run(buf53, primals_27, buf19, buf40, 131072, grid=grid(131072), stream=stream0)
        return (buf53, primals_1, primals_11, primals_21, primals_27, buf0, buf3, buf4, buf16, buf20, buf24, buf25, reinterpret_tensor(primals_29, (128, 1024), (1024, 1), 0), buf32, buf33, buf34, buf37, buf41, buf45, buf46, buf47, buf48, buf52, reinterpret_tensor(primals_25, (1024, 4096), (4096, 1), 0), reinterpret_tensor(primals_23, (4096, 1024), (1024, 1), 0), buf54, reinterpret_tensor(primals_19, (1024, 1024), (1024, 1), 0), reinterpret_tensor(buf35, (16, 128, 128), (16384, 1, 128), 0), reinterpret_tensor(buf30, (16, 64, 128), (8192, 1, 64), 0), reinterpret_tensor(buf29, (16, 64, 128), (8192, 1, 64), 0), reinterpret_tensor(buf31, (16, 128, 64), (8192, 64, 1), 0), reinterpret_tensor(primals_17, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_15, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_13, (1024, 1024), (1024, 1), 0), buf55, reinterpret_tensor(primals_9, (1024, 1024), (1024, 1), 0), reinterpret_tensor(buf14, (16, 128, 128), (16384, 1, 128), 0), reinterpret_tensor(buf9, (16, 64, 128), (8192, 1, 64), 0), buf56, reinterpret_tensor(buf8, (16, 64, 128), (8192, 1, 64), 0), reinterpret_tensor(buf10, (16, 128, 64), (8192, 64, 1), 0), reinterpret_tensor(primals_7, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_5, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_3, (1024, 1024), (1024, 1), 0), )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_29 = rand_strided((1, 128, 1024), (131072, 1024, 1), device='cuda:0', dtype=torch.float32)
