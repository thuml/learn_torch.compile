...
def __guard_1_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140346494402416)) \
        and (L['self'].training == True) \
        and (hasattr(L['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['___stack0'], 134717664)) \
...
# Note: please refer to the graph code in __compiled_fn_51*.py.
# Captured Graph: Dynamo generated graph (debuggable when using eager backend).
# Joint graph: joint forward+backward graph from aot autograd.
# Forward graph: forward graph from aot autograd (debuggable when using aot_eager backend).
# Backward graph: backward graph from aot autograd (debuggable when using aot_eager backend).
# AFTER XXX: graph processed by inductor (not debuggable).
def __compiled_fn_51(*args, **kwargs):
    pass
...
def __transformed_code_1_for_resume_in_forward(___stack0, self, labels, return_dict):
    ...
    graph_out_0 = __compiled_fn_51(___stack0.last_hidden_state, labels)
    import importlib
