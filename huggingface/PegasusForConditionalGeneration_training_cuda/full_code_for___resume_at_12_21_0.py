...
def __guard_0_for___resume_at_12_21(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['key'], 7605632)) \
        and (L['key'] == 'last_hidden_state') \
        and (___check_type_id(L['self'], 134668928)) \
        and (L['self'].last_hidden_state is L['value']) \
        and (___check_obj_id(L['__class__'], 114117808)) \
...
def __helper_outer_function():
    ...
    __class__ = None
    def __transformed_code_0_for___resume_at_12_21(___stack0, self, key, value):
        nonlocal __class__
        import importlib
        def __resume_at_16_22(___stack0):
            nonlocal __class__
            return None
        return __resume_at_16_22(importlib.import_module('builtins').super(
            __class__, self).__setattr__(key, value))


# Note: if there is a transformed version below, this function might well not be executed directly. Please check the transformed version if possible.
def __resume_at_12_21(___stack0, self, key, value):
    nonlocal __class__
    super(__class__, self).__setattr__(key, value)
    return None

def transformed___resume_at_12_21(___stack0, self, key, value):
    L = {"___stack0": ___stack0, "self": self, "key": key, "value": value}
    if __guard_0_for___resume_at_12_21(L):
        return __transformed_code_0_for___resume_at_12_21(___stack0, self, key, value)
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_12_21(___stack0, self, key, value)

#============ end of __resume_at_12_21 ============#
