...
def __resume_at_226_19(___stack0, return_dict, encoder_outputs):
    'Failed to decompile.'

def transformed___resume_at_226_19(___stack0, return_dict, encoder_outputs):
    L = {"___stack0": ___stack0, "return_dict": return_dict, "encoder_outputs": encoder_outputs}
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_226_19(___stack0, return_dict, encoder_outputs)

#============ end of __resume_at_226_19 ============#

def __guard_0_for_resume_in_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 139946272256544)) \
        and (L['self'].training == True) \
        and (___check_type_id(L['___stack0'], 158195824)) \
