...
def __resume_at_108_45(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    'Failed to decompile.'

def transformed___resume_at_108_45(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_45(___stack0, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_108_45 ============#

def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 171515728)) \
...
def __transformed_code_3_for_resume_in_forward_and_backward_pass(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_45(self.optimizer_step(), mod, collect_outputs,
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 171515728)) \
        and (___check_obj_id(L['___stack0'], 40224768)) \
        and (___check_type_id(L['___stack1'], 158151488)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 157248512)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139946272245696)) \
        ...
        and (___check_obj_id(L['self'].autocast, 40224768)) \
...
def __resume_at_20_1(___stack0, self, mod, collect_outputs, cloned_inputs):
    with self.autocast() as __temp_104:
        __temp_106 = {}
        __temp_106.update(cloned_inputs)
        pred = mod(*(), **__temp_106)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def __resume_at_6_0(___stack0, self, mod, collect_outputs):
    ...
    with self.autocast() as __temp_116:
        __temp_118 = {}
        __temp_118.update(cloned_inputs)
        pred = mod(*(), **__temp_118)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_129:
        __temp_131 = {}
        __temp_131.update(cloned_inputs)
        pred = mod(*(), **__temp_131)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
