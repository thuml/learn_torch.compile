...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 158443040)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139631479290752)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 158443040)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'decoder_input_ids', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 27151424)) \
        and (___check_type_id(L['self'].grad_scaler, 144133360)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.long, 139633471178496)) \
        and (G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_torch_dot_nn_dot_modules_dot_module']._global_backward_hooks, 7489504)) \
        and (set(G['__import_torch_dot_nn_dot_modules_dot_module']._global_backward_hooks.keys()) == set()) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float16, 139633471178496)) \
        and (G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.float32, 139633471178496)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_t5_dot_modeling_t5'].torch.bfloat16, 139633471178496)) \
...
def __guard_0_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['___stack0'].keys()) == {'labels', 'decoder_input_ids', 'input_ids'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139628209511952))) \
        and (___compile_config_hash() == 'b22b012cccec369989938c89027da452') \
        and (not ___needs_nopython())
...
def __guard_0_for_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['inputs'].keys()) == {'labels', 'decoder_input_ids', 'input_ids'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139628209511952))) \
        and (___compile_config_hash() == 'b22b012cccec369989938c89027da452') \
        and (not ___needs_nopython())
