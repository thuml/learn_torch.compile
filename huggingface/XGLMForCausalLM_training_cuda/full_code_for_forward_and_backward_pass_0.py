...
def __resume_at_108_40(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    'Failed to decompile.'

def transformed___resume_at_108_40(___stack0, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_40(___stack0, mod, collect_outputs, cloned_inputs, pred, loss)

#============ end of __resume_at_108_40 ============#

def __guard_3_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 165375104)) \
...
def __transformed_code_3_for_resume_in_forward_and_backward_pass(___stack0, self, mod, collect_outputs, cloned_inputs, pred, loss):
    ...
    return __resume_at_108_40(self.optimizer_step(), mod, collect_outputs,
...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_type_id(L['self'], 165375104)) \
        and (___check_obj_id(L['___stack0'], 34085760)) \
        and (___check_type_id(L['___stack1'], 151993856)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 151114640)) \
...
def forward_and_backward_pass(self, mod, inputs, collect_outputs):
    ...
    self.optimizer_zero_grad(mod)
    with self.autocast() as __temp_106:
        __temp_108 = {}
        __temp_108.update(cloned_inputs)
        pred = mod(*(), **__temp_108)
        loss = self.compute_loss(pred)
    self.grad_scaler.scale(loss).backward()
    self.optimizer_step()
