...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140700744651664)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 145408928)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'labels', 'decoder_input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 14126608)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140697476603408))) \
        and (___compile_config_hash() == 'a2debf23689d7dc57ac39411cc041ee0') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.long, 140702738007808)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float16, 140702738007808)) \
        and (G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float32, 140702738007808)) \
        and (G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.float32 == torch.float32) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mt5_dot_modeling_mt5'].torch.bfloat16, 140702738007808)) \
