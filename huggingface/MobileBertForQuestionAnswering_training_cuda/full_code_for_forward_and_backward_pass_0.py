...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140478715858656)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 170393152)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'start_positions', 'end_positions'}) \
        and (___check_obj_id(L['self'].autocast, 39103488)) \
        and (___check_type_id(L['self'].grad_scaler, 156124112)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 100965408)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140478695153168))) \
        and (___compile_config_hash() == '5a049dad893bf116daf0b7b12b6a9f61') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mobilebert_dot_modeling_mobilebert'].torch.long, 140483957008128)) \
