...
def forward(self, primals_3: "i64[1, 1024]", view: "f32[1024, 768]", sub_1: "f32[1024, 50005]", convert_element_type: "f32[]", permute_3: "f32[50005, 768]", tangents_1: "f32[]", tangents_2: "f32[1, 1024, 50005]"):
    ...
    full_default_3: "f32[1024, 50005]" = torch.ops.aten.full.default([1024, 50005], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    scatter: "f32[1024, 50005]" = torch.ops.aten.scatter.value(full_default_3, 1, where_2, -1.0);  full_default_3 = where_2 = None
    ...
    exp_1: "f32[1024, 50005]" = torch.ops.aten.exp.default(sub_1);  sub_1 = None
    ...
    view_4: "f32[1, 1024, 50005]" = torch.ops.aten.reshape.default(sub_2, [1, 1024, 50005]);  sub_2 = None
    
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/plbart/modeling_plbart.py:1718, code: loss = loss_fct(logits.view(-1, self.config.vocab_size), labels.view(-1))
    add: "f32[1, 1024, 50005]" = torch.ops.aten.add.Tensor(tangents_2, view_4);  tangents_2 = view_4 = None
    ...
    view_5: "f32[1024, 50005]" = torch.ops.aten.reshape.default(add, [1024, 50005]);  add = None
    ...
    view_6: "f32[1, 1024, 768]" = torch.ops.aten.reshape.default(mm_2, [1, 1024, 768]);  mm_2 = None
