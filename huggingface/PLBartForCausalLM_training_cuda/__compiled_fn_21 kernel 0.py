...
# kernel path: /tmp/torchinductor_youkaichao/j5/cj5y36jgkbauuavt7yqsjb45x2iuynnwnkcfytjlyu7o5orkd7nu.py
...
# kernel path: /tmp/torchinductor_youkaichao/lk/clkrp6ljbx3t6otcmlgqsxltn2iswlxbf324ntp75hbi62lz2uf2.py
...
def triton_(in_ptr0, in_ptr1, in_ptr2, in_ptr3, in_ptr4, in_ptr5, out_ptr1, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp15 = tl.load(in_ptr4 + (r1 + (50005*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp16 = tl.load(in_ptr0 + (r1 + (50005*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp27 = tl.load(in_ptr5 + (r1 + (50005*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        ...
        tmp30 = tmp26 - tmp29
        tmp31 = tmp15 + tmp30
        tl.store(out_ptr1 + (r1 + (50005*x0)), tmp31, rmask & xmask)
''')
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        triton_poi_fused_nll_loss_backward_nll_loss_forward_0.run(buf0, 51205120, grid=grid(51205120), stream=stream0)
        buf1 = empty_strided((1024, 1), (1, 1024), device='cuda', dtype=torch.int64)
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
        triton_poi_fused_nll_loss_backward_nll_loss_forward_1.run(primals_3, buf1, 1024, grid=grid(1024), stream=stream0)
        aten.scatter_(buf0,1,buf1,-1.0)
        del buf1
        buf5 = empty((1, 1024, 50005), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax_backward_data, aten.add, aten.nll_loss_backward, aten.nll_loss_forward]
        triton_red_fused__log_softmax_backward_data_add_nll_loss_backward_nll_loss_forward_2.run(buf0, primals_3, tangents_1, convert_element_type, tangents_2, sub_1, buf5, 1024, 50005, grid=grid(1024), stream=stream0)
        del buf0
        ...
        del tangents_1
        del tangents_2
        buf6 = empty((50005, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf5, (50005, 1024), (1, 50005), 0), view, out=buf6)
        del view
        buf7 = empty((1024, 768), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    sub_1 = rand_strided((1024, 50005), (50005, 1), device='cuda:0', dtype=torch.float32)
    convert_element_type = rand_strided((), (), device='cuda:0', dtype=torch.float32)
    permute_3 = rand_strided((50005, 768), (768, 1), device='cuda:0', dtype=torch.float32)
    tangents_1 = rand_strided((), (), device='cuda:0', dtype=torch.float32)
    tangents_2 = rand_strided((1, 1024, 50005), (51205120, 50005, 1), device='cuda:0', dtype=torch.float32)
