...
# kernel path: /tmp/torchinductor_youkaichao/po/cpoybd5zlmd43bwbn4ankntdug6ekdv2rqtlglxsyctcypu4feki.py
...
def triton_(in_ptr0, out_ptr2, xnumel, rnumel, XBLOCK : tl.constexpr, RBLOCK : tl.constexpr):
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp4 = tl.load(in_ptr0 + (r1 + (50005*x0)), rmask & xmask, eviction_policy='evict_last', other=0.0)
        tmp5 = tmp4 - tmp2
        tmp6 = tl.exp(tmp5)
    ...
    for roffset in range(0, rnumel, RBLOCK):
        ...
        rmask = rindex < rnumel
        r1 = rindex
        tmp10 = tl.load(in_ptr0 + (r1 + (50005*x0)), rmask & xmask, eviction_policy='evict_first', other=0.0)
        tmp11 = tmp10 - tmp2
        tmp12 = tl.log(tmp8)
        tmp13 = tmp11 - tmp12
        tl.store(out_ptr2 + (r1 + (50005*x0)), tmp13, rmask & xmask)
''')
...
def call(args):
    ...
    assert_size_stride(primals_2, (1, 1024, 768), (786432, 768, 1))
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        extern_kernels.mm(reinterpret_tensor(primals_2, (1024, 768), (768, 1), 0), reinterpret_tensor(primals_1, (768, 50005), (1, 768), 0), out=buf0)
        buf3 = empty((1024, 50005), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        stream0 = get_cuda_stream(0)
        triton_red_fused__log_softmax_0.run(buf0, buf3, 1024, 50005, grid=grid(1024), stream=stream0)
        ...
        return (buf7, reinterpret_tensor(buf0, (1, 1024, 50005), (51205120, 50005, 1), 0), primals_3, reinterpret_tensor(primals_2, (1024, 768), (768, 1), 0), buf3, buf5, reinterpret_tensor(primals_1, (50005, 768), (768, 1), 0), )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_2 = rand_strided((1, 1024, 768), (786432, 768, 1), device='cuda:0', dtype=torch.float32)
