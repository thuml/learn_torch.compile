...
def forward(self, primals_3: "i64[1, 2048]", view: "f32[2048, 768]", sub_1: "f32[2047, 50272]", convert_element_type: "f32[]", permute_3: "f32[50272, 768]", tangents_1: "f32[]", tangents_2: "f32[1, 2048, 50272]"):
    ...
    alias: "f32[2047, 50272]" = torch.ops.aten.alias.default(sub_1);  sub_1 = None
    ...
    full_default_3: "f32[2047, 50272]" = torch.ops.aten.full.default([2047, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    ...
    full_default_5: "f32[1, 2047, 50272]" = torch.ops.aten.full.default([1, 2047, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    slice_scatter: "f32[1, 2047, 50272]" = torch.ops.aten.slice_scatter.default(full_default_5, view_4, 2, 0, 9223372036854775807);  full_default_5 = view_4 = None
    full_default_6: "f32[1, 2048, 50272]" = torch.ops.aten.full.default([1, 2048, 50272], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    slice_scatter_1: "f32[1, 2048, 50272]" = torch.ops.aten.slice_scatter.default(full_default_6, slice_scatter, 1, 0, -1);  full_default_6 = slice_scatter = None
