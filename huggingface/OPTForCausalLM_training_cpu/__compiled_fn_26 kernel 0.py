        ...
        }
        {
            ...
            for(long x0=static_cast<long>(0L); x0<static_cast<long>(2047L); x0+=static_cast<long>(1L))
            {
                for(long x1=static_cast<long>(0L); x1<static_cast<long>(50272L); x1+=static_cast<long>(8L))
                {
                    ...
                    auto tmp7 = tmp3 - tmp6;
                    tmp7.store(out_ptr2 + static_cast<long>(x1 + (50272L*x0)));
                }
            }
        }
        #pragma omp single
            ...
            {
                {
                    ...
                    for(long x0=static_cast<long>(0L); x0<static_cast<long>(2047L); x0+=static_cast<long>(1L))
                    {
                        ...
                        auto tmp6 = decltype(tmp5)(tmp5 + 50272);
                        auto tmp7 = tmp5 < 0;
                        auto tmp8 = tmp7 ? tmp6 : tmp5;
                        TORCH_CHECK((0 <= tmp8) & (tmp8 < 50272L), "index out of bounds: 0 <= tmp8 < 50272L")
                        auto tmp9 = out_ptr2[static_cast<long>(tmp8 + (50272L*x0))];
...
def call(args):
    ...
    buf0 = empty((2048, 50272), device='cpu', dtype=torch.float32)
    ...
    buf1 = empty_strided((2047, 1), (1, 2047), device='cpu', dtype=torch.float32)
    buf2 = empty_strided((2047, 1), (1, 2047), device='cpu', dtype=torch.float32)
    buf3 = empty((2047, 50272), device='cpu', dtype=torch.float32)
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    primals_1 = rand_strided((50272, 768), (768, 1), device='cpu', dtype=torch.float32)
    primals_2 = rand_strided((1, 2048, 768), (1572864, 768, 1), device='cpu', dtype=torch.float32)
