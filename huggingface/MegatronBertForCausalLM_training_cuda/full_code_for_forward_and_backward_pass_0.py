...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 145687744)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140100991810816)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 145687744)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 14396480)) \
        and (___check_type_id(L['self'].grad_scaler, 131388384)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 76257088)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_megatron_bert_dot_modeling_megatron_bert'].torch.long, 140102974576384)) \
