...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139986083202096)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 162409312)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 31120464)) \
        and (hasattr(L['inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs']['input_ids'], 92982656)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139982813109776))) \
        and (___compile_config_hash() == '19cddfe453fa3a186ff965de4970695b') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.long, 139988074886912)) \
        and (G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.float16, 139988074886912)) \
