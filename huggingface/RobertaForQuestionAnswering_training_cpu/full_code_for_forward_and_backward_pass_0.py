...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139870020077552)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 155703536)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'end_positions', 'input_ids', 'start_positions'}) \
        and (___check_obj_id(L['self'].autocast, 24415680)) \
        and (___check_type_id(L['self'].grad_scaler, 141440096)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 86276240)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139866742136336))) \
        and (___compile_config_hash() == '77f12b324fa8d5168549bab3f2643683') \
