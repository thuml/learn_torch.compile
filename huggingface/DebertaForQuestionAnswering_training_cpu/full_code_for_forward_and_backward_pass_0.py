...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140047008391584)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 167142208)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'start_positions', 'end_positions'}) \
        and (___check_obj_id(L['self'].autocast, 35859904)) \
        and (___check_type_id(L['self'].grad_scaler, 152866080)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 97721888)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140043947023888))) \
        and (___compile_config_hash() == 'b93d639b95987d953a321899a80f50e8') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.long, 140049208436480)) \
        and (G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.float, 140049208436480)) \
        and (G['__import_transformers_dot_models_dot_deberta_dot_modeling_deberta'].torch.float == torch.float32) \
...
def __guard_0_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['___stack0'].keys()) == {'input_ids', 'start_positions', 'end_positions'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140043947023888))) \
        and (___compile_config_hash() == 'b93d639b95987d953a321899a80f50e8') \
        and (not ___needs_nopython())
...
def __guard_0_for_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['inputs'].keys()) == {'input_ids', 'start_positions', 'end_positions'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140043947023888))) \
        and (___compile_config_hash() == 'b93d639b95987d953a321899a80f50e8') \
        and (not ___needs_nopython())
