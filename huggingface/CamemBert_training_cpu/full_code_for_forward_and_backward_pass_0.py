...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140657492964496)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 145181776)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 13892736)) \
        and (___check_type_id(L['self'].grad_scaler, 130889344)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 75753104)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140654224006672))) \
        and (___compile_config_hash() == '1e02e473a59f0b14524eb2e30d130949') \
        ...
        and (___check_obj_id(G['__import_transformers_dot_models_dot_camembert_dot_modeling_camembert'].gelu, 140654321532784)) \
        and (G['__import_transformers_dot_models_dot_camembert_dot_modeling_camembert'].gelu.training == True) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_camembert_dot_modeling_camembert'].create_position_ids_from_input_ids.__defaults__[0], 7640416)) \
        and (G['__import_transformers_dot_models_dot_camembert_dot_modeling_camembert'].create_position_ids_from_input_ids.__defaults__[0] == 0) \
