...
def __guard_1_for_forward(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['self'], 140557734402112)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['__class__'], 160832912)) \
        and (___check_type_id(L['input_ids_shape'], 140559849230048)) \
        ...
        and (___check_type_id(G['torch'].long, 140559849223936)) \
...
def __helper_outer_function():
    ...
    __class__ = None
    def __transformed_code_1_for_forward(self, input_ids_shape, past_key_values_length):
...
def transformed_forward(self, input_ids_shape, past_key_values_length):
    ...
    if __guard_1_for_forward(L):
        return __transformed_code_1_for_forward(self, input_ids_shape, past_key_values_length)
