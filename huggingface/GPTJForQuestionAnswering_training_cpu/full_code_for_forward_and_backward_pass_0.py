...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140341120202544)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 166444720)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'start_positions', 'input_ids', 'end_positions'}) \
        and (___check_obj_id(L['self'].autocast, 35155456)) \
        and (___check_type_id(L['self'].grad_scaler, 152155600)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140337850867216))) \
        and (___compile_config_hash() == '18762f6ddb3324fd192fc4003bcf53a2') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_gptj_dot_modeling_gptj'].torch.long, 140343112365824)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_gptj_dot_modeling_gptj'].torch.float32, 140343112365824)) \
...
def __guard_0_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (set(L['___stack0'].keys()) == {'start_positions', 'input_ids', 'end_positions'}) \
        and (utils_device.CURRENT_DEVICE == None) \
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140337850867216))) \
        and (___compile_config_hash() == '18762f6ddb3324fd192fc4003bcf53a2') \
        and (not ___needs_nopython())
