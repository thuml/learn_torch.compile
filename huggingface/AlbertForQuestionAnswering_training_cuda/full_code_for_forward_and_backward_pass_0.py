...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 142549920)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 139751831040208)) \
        ...
        and (___check_obj_id(L['self'].autocast, 11263040)) \
        and (___check_type_id(L['self'].grad_scaler, 128262160)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 73124656)) \
        ...
        and (G['__import_transformers_dot_modeling_utils'].ENV_VARS_TRUE_VALUES == {'YES', '1', 'TRUE', 'ON'}) \
