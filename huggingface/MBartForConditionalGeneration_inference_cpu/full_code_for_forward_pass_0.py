...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139767605232736)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 158750064)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 27459088)) \
        and (hasattr(L['inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs']['input_ids'], 89319840)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139764338794000))) \
        and (___compile_config_hash() == '3ff608de95dd32b0b3348aa98f21b14e') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.long, 139769600710400)) \
        and (G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.float16, 139769600710400)) \
        and (G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart'].torch.float16 == torch.float16) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart']._make_causal_mask.__defaults__[0], 7640416)) \
        and (G['__import_transformers_dot_models_dot_mbart_dot_modeling_mbart']._make_causal_mask.__defaults__[0] == 0) \
