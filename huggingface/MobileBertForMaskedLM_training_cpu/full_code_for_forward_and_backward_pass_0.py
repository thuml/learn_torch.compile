...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139632799285744)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 166283312)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 34995616)) \
        and (___check_type_id(L['self'].grad_scaler, 152026912)) \
        and (hasattr(L['cloned_inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 96855568)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139629610950160))) \
        and (___compile_config_hash() == '2650cdddf69ee473f6e8851e8ad19fb4') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mobilebert_dot_modeling_mobilebert'].torch.long, 139634872510208)) \
