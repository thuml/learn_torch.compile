...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140674561014800)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 145576320)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 14289872)) \
        and (hasattr(L['inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs']['input_ids'], 76151712)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140671290383888))) \
        and (___compile_config_hash() == 'fefd0fc4cf9e779ef0723643de180c8d') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_bart_dot_modeling_bart'].torch.long, 140676551911168)) \
