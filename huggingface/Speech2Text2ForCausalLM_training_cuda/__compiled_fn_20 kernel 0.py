...
# kernel path: /tmp/torchinductor_youkaichao/s7/cs77es23fvjy7tsxpsxtpiffd2xxdpbxeei3q3ld5mu7atotjvdm.py
...
# kernel path: /tmp/torchinductor_youkaichao/tg/ctgp4452gfb5iedlnpicgxhc2bl6imffbfidtabdgys7l24rytdw.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => amax
...
# kernel path: /tmp/torchinductor_youkaichao/sp/cspm5vcoenwjfv5muayhzgpv5gowsmrvvddthvr46k5yifepct3l.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => exp, sub, sum_1
...
# kernel path: /tmp/torchinductor_youkaichao/vc/cvcm5lnlp5zbbu7zotlhdr46idurksu65hlycf2z6u4ajeqxj6yn.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => exp, sub, sum_1
...
# kernel path: /tmp/torchinductor_youkaichao/l4/cl4i72uy2k7koqzl7y7rymbngk6wdz5j6lctufmgnbl52lddymto.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax]
# loss => log, sub, sub_1
...
# kernel path: /tmp/torchinductor_youkaichao/uh/cuhvpy4nli6zdbm7zpchnk4nii46vekuaylfb53zqliropzs5ajc.py
...
def triton_(in_out_ptr0, in_ptr0, in_ptr1, out_ptr1, xnumel, rnumel, XBLOCK : tl.constexpr):
    ...
    tl.device_assert((0 <= tmp12) & (tmp12 < 10000), "index out of bounds: 0 <= tmp12 < 10000")
    tmp13 = tl.load(in_ptr1 + (tmp12 + (10000*r0)), rmask, eviction_policy='evict_last', other=0.0)
    tmp14 = -tmp13
    tmp15 = 0.0
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        buf1 = empty_strided((128, 1, 2), (2, 256, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        stream0 = get_cuda_stream(0)
        triton_red_fused__log_softmax_0.run(buf0, buf1, 256, 5000, grid=grid(256), stream=stream0)
        buf2 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_per_fused__log_softmax_1.run(buf1, buf2, 128, 2, grid=grid(128), stream=stream0)
        buf3 = buf1; del buf1  # reuse
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_red_fused__log_softmax_2.run(buf0, buf2, buf3, 256, 5000, grid=grid(256), stream=stream0)
        buf4 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_per_fused__log_softmax_3.run(buf3, buf4, 128, 2, grid=grid(128), stream=stream0)
        del buf3
        buf5 = empty((128, 10000), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax]
        triton_poi_fused__log_softmax_4.run(buf0, buf2, buf4, buf5, 1280000, grid=grid(1280000), stream=stream0)
        del buf2
        del buf4
        ...
        return (buf9, reinterpret_tensor(buf0, (1, 128, 10000), (1280000, 10000, 1), 0), primals_3, reinterpret_tensor(primals_2, (128, 256), (256, 1), 0), buf5, buf7, reinterpret_tensor(primals_1, (10000, 256), (256, 1), 0), )


def benchmark_compiled_module(times=10, repeat=10):
    from torch._dynamo.testing import rand_strided
    from torch._inductor.utils import print_performance
    primals_1 = rand_strided((10000, 256), (256, 1), device='cuda:0', dtype=torch.float32)
    primals_2 = rand_strided((1, 128, 256), (32768, 256, 1), device='cuda:0', dtype=torch.float32)
