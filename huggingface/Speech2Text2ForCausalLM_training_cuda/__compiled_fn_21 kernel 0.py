...
# kernel path: /tmp/torchinductor_youkaichao/6l/c6lfhcu3rzxignjhcm4uogsw3ygoc5252npi7ik2vnr2tvru5hi6.py
...
# kernel path: /tmp/torchinductor_youkaichao/qo/cqophgh2br536b7fylrz47sgsz3htwyu56v35mpfvu7atfz3rh4d.py
...
@reduction(
    ...
    filename=__file__,
    triton_meta={'signature': {0: '*fp32', 1: '*i64', 2: '*fp32', 3: '*fp32', 4: '*fp32', 5: 'i32', 6: 'i32'}, 'device': 0, 'device_type': 'cuda', 'constants': {}, 'configs': [instance_descriptor(divisible_by_16=(0, 1, 2, 3, 4, 5), equal_to_1=(), ids_of_folded_args=(), divisible_by_8=(5, 6))]},
...
# kernel path: /tmp/torchinductor_youkaichao/4x/c4xobmfbs7x6imdu6q2p6vmrhsssipgfvortfivukjxcuibll7oh.py
# Source Nodes: [loss], Original ATen: [aten._log_softmax_backward_data, aten.nll_loss_backward, aten.nll_loss_forward]
# loss => full_default_1
...
# kernel path: /tmp/torchinductor_youkaichao/vc/cvcuzcovtp46ckqt3typnhvmx2zrj6xfdqvqmfi4wth3tbxrkxye.py
...
def call(args):
    ...
    assert_size_stride(sub_1, (128, 10000), (10000, 1))
    assert_size_stride(convert_element_type, (), ())
    assert_size_stride(permute_3, (10000, 256), (256, 1))
    assert_size_stride(tangents_1, (), ())
    assert_size_stride(tangents_2, (1, 128, 10000), (1280000, 10000, 1))
    with torch.cuda._DeviceGuard(0):
        torch.cuda.set_device(0) # no-op to ensure context
        buf0 = empty((128, 10000), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_nll_loss_backward_nll_loss_forward_0.run(buf0, 1280000, grid=grid(1280000), stream=stream0)
        buf1 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.int64)
        # Source Nodes: [loss], Original ATen: [aten.nll_loss_backward, aten.nll_loss_forward]
        triton_poi_fused_nll_loss_backward_nll_loss_forward_1.run(primals_3, buf1, 128, grid=grid(128), stream=stream0)
        aten.scatter_(buf0,1,buf1,-1.0)
        del buf1
        buf4 = empty_strided((128, 1, 2), (2, 256, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax_backward_data, aten.nll_loss_backward, aten.nll_loss_forward]
        triton_red_fused__log_softmax_backward_data_nll_loss_backward_nll_loss_forward_2.run(buf0, primals_3, tangents_1, convert_element_type, buf4, 256, 5000, grid=grid(256), stream=stream0)
        buf5 = empty_strided((128, 1), (1, 128), device='cuda', dtype=torch.float32)
        # Source Nodes: [loss], Original ATen: [aten._log_softmax_backward_data, aten.nll_loss_backward, aten.nll_loss_forward]
        triton_per_fused__log_softmax_backward_data_nll_loss_backward_nll_loss_forward_3.run(buf4, buf5, 128, 2, grid=grid(128), stream=stream0)
        del buf4
        buf3 = empty((128, 10000), device='cuda', dtype=torch.float32)
        buf6 = reinterpret_tensor(buf3, (1, 128, 10000), (1280000, 10000, 1), 0); del buf3  # reuse
        # Source Nodes: [], Original ATen: [aten.add]
        triton_poi_fused_add_4.run(buf6, tangents_2, buf0, primals_3, tangents_1, convert_element_type, sub_1, buf5, 1280000, grid=grid(1280000), stream=stream0)
        del buf0
        del buf5
        ...
        del tangents_1
        del tangents_2
        buf7 = empty((10000, 256), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf6, (10000, 128), (1, 10000), 0), view, out=buf7)
        del view
        buf8 = empty((128, 256), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: [aten.mm]
        extern_kernels.mm(reinterpret_tensor(buf6, (128, 10000), (10000, 1), 0), permute_3, out=buf8)
        del buf6
        del permute_3
        return (reinterpret_tensor(buf7, (10000, 256), (256, 1), 0), reinterpret_tensor(buf8, (1, 128, 256), (32768, 256, 1), 0), None, )
...
def benchmark_compiled_module(times=10, repeat=10):
    ...
    sub_1 = rand_strided((128, 10000), (10000, 1), device='cuda:0', dtype=torch.float32)
    convert_element_type = rand_strided((), (), device='cuda:0', dtype=torch.float32)
    permute_3 = rand_strided((10000, 256), (256, 1), device='cuda:0', dtype=torch.float32)
    tangents_1 = rand_strided((), (), device='cuda:0', dtype=torch.float32)
    tangents_2 = rand_strided((1, 128, 10000), (1280000, 10000, 1), device='cuda:0', dtype=torch.float32)
