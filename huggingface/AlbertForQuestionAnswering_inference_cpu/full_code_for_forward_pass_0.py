...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140461860221424)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 153981648)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'input_ids', 'end_positions', 'start_positions'}) \
        and (___check_obj_id(L['self'].autocast, 22687312)) \
        and (___check_type_id(L['inputs']['input_ids'], 84547792)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140461861019152))) \
        and (___compile_config_hash() == '0fdde234eed51895ae5a56dbad039c0a') \
