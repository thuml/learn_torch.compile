...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140463216360080)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 150077872)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 18783824)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140459949825552))) \
        and (___compile_config_hash() == '8f7c0bd96bbc04bf6f58ab7910ce9fa0') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_blenderbot_small_dot_modeling_blenderbot_small'].torch.long, 140465211561728)) \
...
def __transformed_code_0_for_forward_pass(self, mod, inputs, collect_outputs):
    ...
    return importlib.import_module('transformers.modeling_outputs'
        ...
        (graph_out_0[16], graph_out_0[17])), hidden_states=None, attentions=
