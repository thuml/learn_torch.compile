...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139805888790336)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 147213536)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'decoder_input_ids', 'input_ids', 'labels'}) \
        and (___check_obj_id(L['self'].autocast, 15924736)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139802865393168))) \
        and (___compile_config_hash() == '7f2dd66f9db958155de50b536270406a') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_pegasus_dot_modeling_pegasus'].torch.long, 139808127350528)) \
        and (G['__import_transformers_dot_models_dot_pegasus_dot_modeling_pegasus'].torch.long == torch.int64) \
        and (___check_type_id(G['__import_transformers_dot_models_dot_pegasus_dot_modeling_pegasus'].torch.float16, 139808127350528)) \
