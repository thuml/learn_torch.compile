...
def __guard_0_for_sgd(L):
    return (___guarded_code.valid) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140599198244368))) \
        and (___compile_config_hash() == 'db78b5ca5a93382ec23de42d0820edda') \
...
def sgd(params, d_p_list, momentum_buffer_list, has_sparse_grad, foreach, weight_decay, momentum, lr, dampening, nesterov, maximize):
    if foreach is None:
        if not torch.jit.is_scripting():
            __temp_735 = _default_to_fused_or_foreach(params, differentiable=
                False, use_fused=False)
            _ = __temp_735[0]
            foreach = __temp_735[1]
        else:
            foreach = False
    if foreach:
        if torch.jit.is_scripting():
