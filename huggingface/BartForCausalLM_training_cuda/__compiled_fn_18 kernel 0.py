...
# kernel path: /tmp/torchinductor_youkaichao/2d/c2dicilhmcisdjuqxyro2vzngxlwfz4kbmfjvktrumu3hu54tdjg.py
...
# kernel path: /tmp/torchinductor_youkaichao/wm/cwmu23fuzyndu6dnja23tatr53vggn4awjn2dwnktmretl7kzrsi.py
...
# kernel path: /tmp/torchinductor_youkaichao/ur/cur6cx5tebtd5od5k4ztphlpxyzozcvzrrkms2rxypqcrqxmr4j3.py
...
def call(args):
    ...
    with torch.cuda._DeviceGuard(0):
        ...
        extern_kernels.mm(reinterpret_tensor(primals_17, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_1, (1024, 1024), (1, 1024), 0), out=buf0)
        buf1 = empty((1024, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_17, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_3, (1024, 1024), (1, 1024), 0), out=buf1)
        buf2 = empty((1, 16, 1024, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [key_states], Original ATen: [aten.clone]
        stream0 = get_cuda_stream(0)
        triton_poi_fused_clone_0.run(buf1, primals_4, buf2, 1048576, grid=grid(1048576), stream=stream0)
        del primals_4
        buf3 = buf1; del buf1  # reuse
        # Source Nodes: [], Original ATen: []
        extern_kernels.mm(reinterpret_tensor(primals_17, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_5, (1024, 1024), (1, 1024), 0), out=buf3)
        buf4 = empty((1, 16, 1024, 64), device='cuda', dtype=torch.float32)
        # Source Nodes: [value_states], Original ATen: [aten.clone]
        triton_poi_fused_clone_0.run(buf3, primals_6, buf4, 1048576, grid=grid(1048576), stream=stream0)
        del primals_6
        buf5 = reinterpret_tensor(buf3, (1, 16, 1024, 64), (1048576, 65536, 64, 1), 0); del buf3  # reuse
        # Source Nodes: [contiguous_2], Original ATen: [aten.clone]
        triton_poi_fused_clone_1.run(buf0, primals_2, buf5, 1048576, grid=grid(1048576), stream=stream0)
        del primals_2
        buf6 = empty((16, 1024, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [attn_weights], Original ATen: [aten.bmm]
        extern_kernels.bmm(reinterpret_tensor(buf5, (16, 1024, 64), (65536, 64, 1), 0), reinterpret_tensor(buf2, (16, 64, 1024), (65536, 1, 64), 0), out=buf6)
        buf9 = empty((16, 1024, 1024), device='cuda', dtype=torch.float32)
        buf34 = empty((16, 1024, 1024), device='cuda', dtype=torch.float32)
        # Source Nodes: [attn_probs, attn_weights_3], Original ATen: [aten._softmax, aten.clone, aten.detach]
        triton_per_fused__softmax_clone_detach_2.run(buf6, primals_18, buf9, buf34, 16384, 1024, grid=grid(16384), stream=stream0)
        del buf6
        del primals_18
        buf10 = reinterpret_tensor(buf0, (16, 1024, 64), (65536, 64, 1), 0); del buf0  # reuse
        ...
        # Source Nodes: [hidden_states_2, l__self___fc1, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward, aten.view]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_view_4.run(primals_17, buf14, primals_9, primals_10, buf19, buf20, buf33, 1024, 1024, grid=grid(1024), stream=stream0)
        ...
        del primals_12
        buf22 = empty((1024, 4096), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_4, hidden_states_6], Original ATen: [aten.gelu, aten.view]
        triton_poi_fused_gelu_view_5.run(buf21, buf22, 4194304, grid=grid(4194304), stream=stream0)
        buf23 = reinterpret_tensor(buf14, (1024, 1024), (1024, 1), 0); del buf14  # reuse
        ...
        buf24 = aten.native_dropout(reinterpret_tensor(buf23, (1, 1024, 1024), (1048576, 1024, 1), 0), 0.1, True)
        buf25 = buf24[0]
        buf26 = buf24[1]
        del buf24
        buf30 = reinterpret_tensor(buf23, (1, 1024, 1024), (1048576, 1024, 1), 0); del buf23  # reuse
        buf31 = empty((1, 1024, 1024), device='cuda', dtype=torch.float32)
        buf32 = empty((1, 1024, 1), device='cuda', dtype=torch.float32)
        # Source Nodes: [hidden_states_8, hidden_states_9, residual_1], Original ATen: [aten.add, aten.native_layer_norm, aten.native_layer_norm_backward]
        triton_per_fused_add_native_layer_norm_native_layer_norm_backward_6.run(buf19, primals_9, primals_10, buf25, primals_15, primals_16, buf30, buf31, buf32, 1024, 1024, grid=grid(1024), stream=stream0)
        del buf25
        del primals_10
        del primals_16
        return (buf31, buf2, buf4, primals_9, primals_15, reinterpret_tensor(primals_17, (1024, 1024), (1024, 1), 0), buf11, buf15, buf19, buf20, buf21, buf22, buf26, buf30, buf32, reinterpret_tensor(primals_13, (1024, 4096), (4096, 1), 0), reinterpret_tensor(primals_11, (4096, 1024), (1024, 1), 0), buf33, reinterpret_tensor(primals_7, (1024, 1024), (1024, 1), 0), reinterpret_tensor(buf9, (16, 1024, 1024), (1048576, 1, 1024), 0), reinterpret_tensor(buf4, (16, 64, 1024), (65536, 1, 64), 0), buf34, reinterpret_tensor(buf5, (16, 64, 1024), (65536, 1, 64), 0), reinterpret_tensor(buf2, (16, 1024, 64), (65536, 64, 1), 0), reinterpret_tensor(primals_5, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_3, (1024, 1024), (1024, 1), 0), reinterpret_tensor(primals_1, (1024, 1024), (1024, 1), 0), )
