...
def __guard_0_for_forward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 140493598342928)) \
        and (L['mod'].training == False) \
        and (___check_type_id(L['self'], 168444832)) \
        and (___check_type_id(L['inputs'], 7638432)) \
        and (set(L['inputs'].keys()) == {'labels', 'input_ids'}) \
        and (___check_obj_id(L['self'].autocast, 37157648)) \
        and (hasattr(L['inputs']['labels'], '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['inputs']['input_ids'], 99019968)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(140490334166544))) \
        and (___compile_config_hash() == '7e7f5697bab602d18caa84133fc26835') \
        ...
        and (___check_obj_id(G['__import_transformers_dot_models_dot_camembert_dot_modeling_camembert'].gelu, 140490431692656)) \
