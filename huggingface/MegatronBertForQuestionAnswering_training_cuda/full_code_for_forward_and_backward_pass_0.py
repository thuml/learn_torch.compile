...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 154212560)) \
...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['mod'], 140357343668992)) \
        ...
        and (___check_obj_id(L['self'].autocast, 22924272)) \
        and (___check_type_id(L['self'].grad_scaler, 139952640)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 84784848)) \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_megatron_bert_dot_modeling_megatron_bert'].torch.long, 140359334762240)) \
