...
def __guard_2_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_type_id(L['self'], 171716096)) \
        and (___check_obj_id(L['___stack0'], 40431040)) \
        and (___check_type_id(L['___stack1'], 158352608)) \
        and (hasattr(L['___stack1'].loss, '_dynamo_dynamic_indices') == False) \
        and (hasattr(L['___stack1'].logits, '_dynamo_dynamic_indices') == False) \
        and (___check_type_id(L['self'].grad_scaler, 157455728)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139621184118288))) \
        and (___compile_config_hash() == '140d06855560227a1ca136d524c2b86a') \
