...
def forward(self, add: "i64[1, 256]", tangents_1: "f32[1, 256, 1024]"):
    ...
    full_default: "b8[1, 256, 1]" = torch.ops.aten.full.default([1, 256, 1], False, dtype = torch.bool, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    ...
    full_default_2: "f32[514, 1024]" = torch.ops.aten.full.default([514, 1024], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    _unsafe_index_put: "f32[514, 1024]" = torch.ops.prims._unsafe_index_put_.default(full_default_2, [add], where, True);  full_default_2 = add = where = None
