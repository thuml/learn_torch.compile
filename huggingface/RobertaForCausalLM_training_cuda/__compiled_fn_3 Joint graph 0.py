...
def forward(self, primals, tangents):
    ...
    alias_12: "f32[511, 50265]" = torch.ops.aten.alias.default(sub_40)
    ...
    where: "i64[511]" = torch.ops.aten.where.self(ne_1, view_269, scalar_tensor);  ne_1 = scalar_tensor = None
    ...
    where_1: "f32[511]" = torch.ops.aten.where.self(ne_2, neg, scalar_tensor_1);  ne_2 = neg = scalar_tensor_1 = None
    ne_3: "b8[511]" = torch.ops.aten.ne.Scalar(view_269, -100)
    ...
    where_2: "i64[511, 1]" = torch.ops.aten.where.self(ne_4, unsqueeze_3, scalar_tensor_2);  ne_4 = scalar_tensor_2 = None
    full_1: "f32[511, 50265]" = torch.ops.aten.full.default([511, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    scatter: "f32[511, 50265]" = torch.ops.aten.scatter.value(full_1, 1, where_2, -1.0);  full_1 = where_2 = None
    ne_5: "b8[511, 1]" = torch.ops.aten.ne.Scalar(unsqueeze_3, -100);  unsqueeze_3 = None
    ...
    where_3: "f32[511, 1]" = torch.ops.aten.where.self(ne_5, div_25, scalar_tensor_3);  ne_5 = div_25 = scalar_tensor_3 = None
    ...
    # File: /workspace/youkaichao/miniconda3/envs/build_torch/lib/python3.10/site-packages/transformers/models/roberta/modeling_roberta.py:986, code: shifted_prediction_scores = prediction_scores[:, :-1, :].contiguous()
    full_2: "f32[1, 511, 50265]" = torch.ops.aten.full.default([1, 511, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    slice_scatter: "f32[1, 511, 50265]" = torch.ops.aten.slice_scatter.default(full_2, view_270, 2, 0, 9223372036854775807);  full_2 = view_270 = None
    full_3: "f32[1, 512, 50265]" = torch.ops.aten.full.default([1, 512, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    slice_scatter_1: "f32[1, 512, 50265]" = torch.ops.aten.slice_scatter.default(full_3, slice_scatter, 1, 0, -1);  full_3 = slice_scatter = None
    full_4: "f32[1, 512, 50265]" = torch.ops.aten.full.default([1, 512, 50265], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    slice_scatter_2: "f32[1, 512, 50265]" = torch.ops.aten.slice_scatter.default(full_4, slice_scatter_1, 0, 0, 9223372036854775807);  full_4 = slice_scatter_1 = None
    ...
    full_5: "f32[512, 768]" = torch.ops.aten.full.default([512, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    _unsafe_index_put: "f32[512, 768]" = torch.ops.aten._unsafe_index_put.default(full_5, [add_1], where_4, True);  full_5 = add_1 = where_4 = None
    ...
    full_6: "f32[2, 768]" = torch.ops.aten.full.default([2, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    _unsafe_index_put_1: "f32[2, 768]" = torch.ops.aten._unsafe_index_put.default(full_6, [expand], where_5, True);  full_6 = expand = where_5 = None
    ...
    full_7: "f32[50265, 768]" = torch.ops.aten.full.default([50265, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cuda', index=0), pin_memory = False)
    _unsafe_index_put_2: "f32[50265, 768]" = torch.ops.aten._unsafe_index_put.default(full_7, [primals_206], where_6, True);  full_7 = primals_206 = where_6 = None
