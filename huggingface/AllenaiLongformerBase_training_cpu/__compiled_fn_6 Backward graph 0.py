...
def forward(self, primals_4: "f32[768]", primals_6: "i64[1, 1024]", full_default: "i64[1, 1024]", add: "i64[1, 1024]", mul_2: "f32[1, 1024, 768]", getitem_3: "b8[1, 1024, 768]", div: "f32[1, 1024, 1]", tangents_1: "f32[1, 1024, 768]"):
    ...
    mul_12: "f32[1, 1024, 768]" = torch.ops.aten.mul.Tensor(clone, mul_2);  mul_2 = None
    ...
    full_default_2: "b8[1, 1024, 1]" = torch.ops.aten.full.default([1, 1024, 1], False, dtype = torch.bool, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    ...
    full_default_4: "f32[1, 768]" = torch.ops.aten.full.default([1, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    _unsafe_index_put: "f32[1, 768]" = torch.ops.aten._unsafe_index_put.default(full_default_4, [full_default], where, True);  full_default_4 = full_default = where = None
    ...
    full_default_6: "f32[4098, 768]" = torch.ops.aten.full.default([4098, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    _unsafe_index_put_1: "f32[4098, 768]" = torch.ops.aten._unsafe_index_put.default(full_default_6, [add], where_1, True);  full_default_6 = add = where_1 = None
    ...
    full_default_8: "f32[50265, 768]" = torch.ops.aten.full.default([50265, 768], 0, dtype = torch.float32, layout = torch.strided, device = device(type='cpu'), pin_memory = False)
    _unsafe_index_put_2: "f32[50265, 768]" = torch.ops.aten._unsafe_index_put.default(full_default_8, [primals_6], where_2, True);  full_default_8 = primals_6 = where_2 = None
