...
def __resume_at_346_7(___stack0, self, return_dict):
    'Failed to decompile.'

def transformed___resume_at_346_7(___stack0, self, return_dict):
    L = {"___stack0": ___stack0, "self": self, "return_dict": return_dict}
    # Note: this function might well not be executed directly. It might well be transformed again, i.e. adding one more guards and transformed code.
    return __resume_at_346_7(___stack0, self, return_dict)

#============ end of __resume_at_346_7 ============#

def __guard_1_for_forward(L):
    return (___guarded_code.valid) \
        and (___check_global_state()) \
        and (___check_obj_id(L['self'], 140541544341968)) \
        and (L['self'].training == True) \
        and (___check_obj_id(L['head_mask'], 7628576)) \
        and (___check_type_id(L['input_ids'], 97891856)) \
        ...
        and (___check_type_id(G['torch'].long, 140543536453376)) \
