...
def __guard_1_for_resume_in_forward_and_backward_pass(L):
    return (___guarded_code.valid) \
        ...
        and (___check_obj_id(L['mod'], 139976442039936)) \
        and (L['mod'].training == True) \
        and (___check_type_id(L['self'], 154454208)) \
        and (___check_type_id(L['cloned_inputs'], 7638432)) \
        and (set(L['cloned_inputs'].keys()) == {'input_ids', 'end_positions', 'start_positions'}) \
        and (___check_obj_id(L['self'].autocast, 23166400)) \
        and (___check_type_id(L['self'].grad_scaler, 140203296)) \
        and (___check_type_id(L['cloned_inputs']['input_ids'], 85026320)) \
        ...
        and ((___skip_backend_check() or ___current_backend() == ___lookup_backend(139976421105168))) \
        and (___compile_config_hash() == '3433aecaad0bb938c60e2bdcbdd403e9') \
        ...
        and (___check_type_id(G['__import_transformers_dot_models_dot_mobilebert_dot_modeling_mobilebert'].torch.long, 139981682931456)) \
